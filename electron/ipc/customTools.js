...
// 创建/更新自定义工具：把 tool.json（manifest）+ tool.js（源码）写入用户工具目录。
// 出于安全考虑，默认 powershell=false；仅当调用方显式 powershell=true 时才授予 shell 能力。
const createTool = ({ id, name, description, code, parameters, powershell }) => {
  if (!id || !/^[a-z0-9][a-z0-9-]*$/.test(String(id))) {
    throw new Error('id 只能包含小写字母、数字、中划线，且以字母或数字开头')
  }
  if (!name || !String(name).trim()) throw new Error('name 不能为空')
  if (!description || !String(description).trim()) throw new Error('description 不能为空')
  if (!code || !String(code).trim()) throw new Error('code 不能为空')

  const base = ensureUserToolDir()
  const target = path.join(base, sanitizeFolderName(String(id).trim()))
  fs.mkdirSync(target, { recursive: true })
  ...
  const manifest = {
    ...
    autoInvoke: true,
    readOnly: false,
    risk: powershell === true ? 'high' : 'medium',
    timeoutMs: 120000,
  ...
  fs.writeFileSync(path.join(target, 'tool.json'), JSON.stringify(manifest, null, 2), 'utf8')
  fs.writeFileSync(path.join(target, 'tool.js'), String(code), 'utf8')
  loadTools() // 刷新缓存，让新工具立即可被 list / call 发现
  return { ok: true, id: manifest.id, dir: target }
}

ipcMain.handle('customTools:list', async () => loadTools())
ipcMain.handle('customTools:create', async (e, payload) => {
  try {
