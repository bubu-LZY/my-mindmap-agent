    ...
    <Transition name="search-dropdown">
      <div v-if="showResults && (results.length > 0 || searching || semanticSearching || searched)" class="search-results">
        <template v-if="results.length">
          ...
          <div
            v-for="(item, idx) in safeTabItems"
...
const onEnter = () => {
  if (debounceTimer) clearTimeout(debounceTimer)
  // 有结果时：回车 = 跳到下一个结果（循环）；无结果时：回车 = 触发搜索
  const items = safeTabItems.value
  if (items.length) {
    const next = currentIndex.value < 0 ? 0 : (currentIndex.value + 1) % items.length
    openResult(items[next])
...
const openResult = (item) => {
  ...
  // 保持结果列表打开，支持连续点击跳转；记录当前索引供回车导航
  currentIndex.value = safeTabItems.value.findIndex(
    r => r.filePath === item.filePath && r.nodeUid === item.nodeUid
  )
}
...
const onBlur = () => {
  // 不再自动关闭下拉：保持结果列表可连续点击/回车导航，仅由「叉号」或 Esc 关闭
}
...
.search-result-item.active {
  ...
  box-shadow: inset 2px 0 0 var(--apple-blue, #007aff);
}
