    ...
    <div class="ref-list" v-if="filteredItems.length > 0" @mouseleave="onListLeave">
      ...
      >
        ...
        <template v-if="mode === 'file'">
          <span class="ref-item-icon">{{ fileIcon(item.path) }}</span>
...
import { getFileExt } from '../utils/refTarget'

// 文件图标：区分「可编辑导图」与「只读文档」，避免列表里全是同一张纸的图标
const FILE_ICONS = {
  smm: '\u{1F9E0}', json: '\u{1F9E0}', xmind: '\u{1F9E0}',
  pdf: '\u{1F4D5}', docx: '\u{1F4D8}', doc: '\u{1F4D8}',
  xlsx: '\u{1F4D7}', xls: '\u{1F4D7}', csv: '\u{1F4D7}', tsv: '\u{1F4D7}',
  pptx: '\u{1F4DA}', ppt: '\u{1F4DA}',
  md: '\u{1F4DD}', markdown: '\u{1F4DD}',
  txt: '\u{1F4C4}', log: '\u{1F4C4}', html: '\u{1F310}', xml: '\u{1F310}'
}

const fileIcon = (filePath) => FILE_ICONS[getFileExt(filePath)] || '\u{1F4C4}'
