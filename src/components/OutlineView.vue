      ...
      >
        <template #default="{ node, data }">
          <span class="custom-tree-node" :class="data.frameClass || ''" :style="data.frameStyle || null">
            ...
            <span v-if="node.isLeaf" class="node-bullet" aria-hidden="true">•</span>
            <span
              class="node-text"
...
// 暂存的展开覆盖指令（在 nextTick 末尾才清空，防止 refresh 被连续触发时第二次失效）
const pendingExpandOverride = ref(null)
// 局部展开：节点右键「展开所有子节点」时，记录要展开的子树的根 uid，刷新时收集其子树全部 uid
const subtreeExpandUid = ref(null)
...
const refresh = () => {
  ...
  if (data) {
    ...
    // 注意：不在此清空，留到 nextTick 末尾清空，避免 refresh 被连续触发两次时第二次失效
    const override = outlineExpandOverride.value
    if (override) {
      pendingExpandOverride.value = override
    }
    const activeOverride = pendingExpandOverride.value
    ...
    if (activeOverride === 'all') {
      ;(treeData.value || []).forEach(collectAllExpanded)
    } else if (activeOverride === 'none') {
      // 收起所有：不展开任何节点
    } else {
      ;(treeData.value || []).forEach(node => collectExpanded(node, 0))
    }
    // 局部展开：把目标子树的全部节点 uid 追加进展开集合（用于节点右键「展开所有子节点」）
    // 注意：subtreeExpandUid 不在此清空，留给 nextTick 里用 el-tree API 强制展开（覆盖 default-expanded-keys 可能失效的情况）
    if (subtreeExpandUid.value) {
      const targetUid = subtreeExpandUid.value
      const findInTreeData = (items) => {
        for (const item of items || []) {
          if (item.uid === targetUid) return item
          const r = findInTreeData(item.children)
          if (r) return r
        }
        return null
      }
      const target = findInTreeData(treeData.value)
      if (target) collectAllExpanded(target)
    }
  ...
  nextTick(() => {
    // 双向同步展开状态：按数据中的 expand 字段，该展开的展开、该收起的收起
    // （解决 defaultExpandedKeys 在数据刷新后不可靠的问题）
    syncExpandState(focusUidForExpand)
    // 局部展开：强制展开目标子树（右键「展开所有子节点」用）
    if (subtreeExpandUid.value) {
      applySubtreeExpand(subtreeExpandUid.value)
      subtreeExpandUid.value = null
    }
    attachTributeToNodes()
    applyOutlineClozeStyles()
    // 最后再清掉一次性展开覆盖标记，确保本次 refresh 完整生效后才允许下一次默认逻辑
    pendingExpandOverride.value = null
  })
}

/**
 * 按 treeData 中的 expand 字段双向同步 el-tree 的展开状态：
 * - expand=true 且当前收起的 → 展开
 * - expand=false 且当前展开的 → 收起
 * 待选节点（新创建节点）的祖先链强制保持展开，避免新节点落在折叠子树内不可见
 */
const syncExpandState = (focusUid = null) => {
  try {
    ...
    const walk = (n) => {
      if (!n) return
      if (n.data && n.childNodes && n.childNodes.length > 0) {
        const shouldExpand = keepExpanded.has(n.data.uid) || n.data.expand !== false
        if (shouldExpand && !n.expanded && typeof n.expand === 'function') {
          n.expand()
        } else if (!shouldExpand && n.expanded && typeof n.collapse === 'function') {
          n.collapse()
        }
      }
...
/** 强制展开指定节点的所有子节点（覆盖 default-expanded-keys 可能失效的情况） */
const applySubtreeExpand = (uid) => {
  const tree = treeRef.value
  if (!tree || !tree.store || !tree.store.nodesMap) return
  const node = tree.store.nodesMap[uid]
  if (!node) return
  const walk = (n) => {
...
/** 展开/收起指定节点的所有子节点（仅作用于该节点子树，不影响其他节点） */
const setNodeExpandByUid = (uid, expand) => {
  if (!props.mindMap?.renderer?.renderTree) return
  ensureUids()
  const result = findNodeAndParent(props.mindMap.renderer.renderTree, uid)
  if (!result || !result.node.children || result.node.children.length === 0) return
  const walk = (node) => {
    if (node.children && node.children.length > 0) {
      node.data.expand = expand
      node.children.forEach(walk)
  ...
  walk(result.node)
  if (expand) subtreeExpandUid.value = uid
  commitData(uid)
}
...
const onContextmenuSelect = ({ action, node }) => {
  ...
  switch (action) {
    ...
    case 'EXPAND_CHILDREN':
      setNodeExpandByUid(uid, true)
      break
    case 'COLLAPSE_CHILDREN':
      setNodeExpandByUid(uid, false)
      break
    case 'RESET_LAYOUT':
