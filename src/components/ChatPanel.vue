  ...
  <div class="chat-panel">
    ...
    <header class="chat-header">
      <div class="header-left">
        <button
          v-if="previousFileInfo"
          class="header-back-btn"
          @click="goBackToPreviousFile"
          :title="`返回：${previousFileInfo.fileName}`"
        >
        ...
        <div v-else class="message-content assistant-content">
          ...
          >
            ...
            <Transition name="reasoning-pop">
              <div v-show="msg.reasoningExpanded" class="reasoning-content" :ref="el => setReasoningRef(msg.id, el)">
      ...
      <div v-if="historyPanelVisible" class="history-panel">
        <div class="history-panel-header">
          ...
          <div class="history-header-actions">
            <button
              v-if="filteredConversations.length > 0 && !conversationSearchKeyword"
        ...
        <div class="history-filter-tabs">
          <button
            class="filter-tab"
            :class="{ active: conversationFilterMode === 'current' }"
            @click="conversationFilterMode = 'current'"
          >
            当前文件
          </button>
          <button
            class="filter-tab"
            :class="{ active: conversationFilterMode === 'global' }"
            @click="conversationFilterMode = 'global'"
          >
            全局对话
          </button>
          <button
            class="filter-tab"
            :class="{ active: conversationFilterMode === 'all' }"
            @click="conversationFilterMode = 'all'"
          >
            全部
          </button>
        </div>
        ...
        <!-- 新建全局对话入口 -->
        <div class="history-new-global">
          <button class="new-global-btn" @click="newGlobalConversation">
        ...
        <div class="history-list">
          <div
            ...
            :key="conv.id"
            class="history-item"
            :class="{ active: currentConversation && conv.id === currentConversation.id, 'is-global': isGlobalConversation(conv) }"
            @click="loadConversation(conv.id)"
          >
            <div class="history-item-content">
              <div class="history-item-title">
                <span v-if="isGlobalConversation(conv)" class="global-badge" title="全局对话（所有文件可见）">🌐</span>
                {{ conv.title || '新对话' }}
              </div>
              <div class="history-item-time">
                {{ formatTime(conv.updatedAt) }}
                <span v-if="!isGlobalConversation(conv) && conversationFilterMode === 'all'" class="conv-file-tag">
                  {{ getConversationFileName(conv.fileId) }}
                </span>
              </div>
            </div>
            <button
          ...
          <div v-if="filteredConversations.length === 0" class="history-empty">
            {{ conversationSearchKeyword ? '没有找到匹配的对话' : '暂无历史记录' }}
          </div>
        </div>
      </div>
    </Transition>
    ...
    >
      ...
      <div class="input-bottom-bar">
        ...
        <button
          ...
          title="工具沉淀：让 AI 引导你创建自己的技能 / 小工具 / 外部服务"
...
} from '../services/multimodalRouter'
import {
  getBestNextAction,
  recordStrategyResult,
  getStrategyMemory
} from '../services/errorStrategyEngine'
import {
  getModelCapability,
...
const emit = defineEmits(['tool-call-status', 'toggle-log-panel', 'log-updated', 'conversation-changed', 'file-created', 'external-file-created', 'file-deleted', 'switch-view', 'open-settings', 'file-renamed', 'update-ai-bind', 'open-reference', 'navigate-to-file'])
...
const conversationFilterMode = ref('current') // current=当前文件, all=全部, global=全局对话
const filteredConversations = computed(() => {
  const kw = conversationSearchKeyword.value.trim()
  ...
  if (kw) {
    ...
    if (conversationFilterMode.value === 'current') searchFileId = fid
    if (conversationFilterMode.value === 'global') searchFileId = ''
    list = searchConversations(kw, searchFileId)
  } else {
    // 普通列表模式
    if (conversationFilterMode.value === 'all') {
      list = loadConversations()
    } else if (conversationFilterMode.value === 'global') {
      list = loadConversationsByFile('', false)
    } else {
      // current：当前文件对话 + 全局对话（全局对话始终可见）
      list = loadConversationsByFile(fid, true)
    }
  }
  return list
})
...
// 文件跳转返回栈：工具调用导致跳转到新文件时，记录跳转前的文件信息
const previousFileInfo = ref(null) // { filePath, fileName }

// 点击返回上一个文件
const goBackToPreviousFile = () => {
  if (!previousFileInfo.value) return
  const target = previousFileInfo.value
  previousFileInfo.value = null
  emit('navigate-to-file', target.filePath, target.fileName)
}
 ...
 */
const analyzeCodeToolUsage = (code) => {
  if (!code || typeof code !== 'string') return []
  const tools = new Set()
  // 匹配 tools.toolName( 或 tools['toolName']( 或 tools["toolName"](
  const re = /tools\s*\.\s*(\w+)\s*\(|tools\s*\[\s*['"]([^'"]+)['"]\s*\]\s*\(/g
  let m
...
/**
 * 判断 run_code 代码是否为只读（仅调用只读工具）
 * @param {string} code
 * @returns {boolean}
 */
const isRunCodeReadOnly = (code) => {
  const usedTools = analyzeCodeToolUsage(code)
  if (usedTools.length === 0) return true // 没调用任何工具，视为安全
  return usedTools.every(t => READONLY_RUN_CODE_TOOLS.has(t))
}
...
const confirmDangerousTool = (toolName, args) => {
  ...
  // run_code 特殊处理：只读代码自动放行
  if (toolName === 'run_code' && isRunCodeReadOnly(args?.code)) {
...
const handleLinkClick = (e) => {
  ...
  e.preventDefault()
  e.stopPropagation()
  // 引用协议跳转：mindmap-file: / mindmap-node:（点击打开对应文件并定位节点）
  if (isReferenceLink(href)) {
    const ref = parseReferenceLink(href)
    if (ref.filePath) emit('open-reference', { filePath: ref.filePath, nodeUid: ref.nodeUid || '' })
    return
  }
...
const retrieveKnowledgeForQuestion = async (question) => {
  ...
  const context = results.slice(0, 20).map((r, i) => {
    const refLink = r.nodeUid ? `mindmap-node:${r.filePath}:${r.nodeUid}` : `mindmap-file:${r.filePath}`
    return `[KB${i + 1}] 文件：${r.fileName}\n内容：${r.snippet || ''}\n引用链接：${refLink}`
  }).join('\n\n')
...
// 新建全局对话（不绑定任何文件，所有文件都可见）
const newGlobalConversation = () => {
  abortActiveGeneration()
  aiStatus.value = 'idle'
  emit('tool-call-status', 'idle')
  if (messages.value.length > 0) {
  ...
  cancelPendingContinue()
  const conv = createConversation('') // fileId 为空表示全局对话
  currentConversation.value = conv
  messages.value = []
  conversationSummary.value = ''
  summaryCoveredCount.value = 0
  conversationFileMap.value.set(conv.id, '')
  setCurrentConversationId(conv.id)
  conversations.value = loadConversations()
  ElMessage.success('已创建全局对话')
}
...
/**
 * 从文件 ID 中提取显示用的文件名
 * @param {string} fileId - 文件路径/ID
 * @returns {string} 文件名（不含路径）
 */
const getConversationFileName = (fileId) => {
  if (!fileId) return '全局'
  // 取最后一段路径作为文件名
...
let pendingExtensionGuide = null // { type } 工具沉淀引导（先分析历史对话）
let extensionGuideType = null    // 当前引导类型（用于消息以原子胶囊展示，不显示完整指令）
...
const selectExtensionType = (type) => {
  // 关闭向导，转交 AI 引导：先分析历史对话，再交互式创建（消息以原子胶囊展示，不显示完整指令）
  extensionWizardVisible.value = false
  extensionWizardStep.value = 0
  pendingExtensionGuide = { type }
  inputText.value = ''
  if (textareaRef.value) textareaRef.value.style.height = 'auto'
  sendMessage()
}

// 三类扩展的通俗称呼（用户可见）与英文标识（AI 识别类型）
const EXTENSION_TYPE_LABEL = { skill: '技能', mcp: '外部服务', tool: '小工具' }
const EXTENSION_TYPE_EN = { skill: 'Skill', mcp: 'MCP 服务', tool: '自定义工具' }

// 三类扩展的创建规范（仅 AI 内部参考，措辞已尽量克制）
const EXTENSION_GUIDE_SPEC = {
  skill: `【技术规范·仅你内部参考，不要原样念给用户】
- 创建工具：create_skill
- 必填：name（英文小写/连字符）、description（何时用）、instructions（分步指令）
- 多轮追问时用 <ask> 块（type=single/multi）渲染成可点按钮`,
  mcp: `【技术规范·仅你内部参考，不要原样念给用户】
- 创建工具：create_mcp_server
- 必填：name、transport（http 需 url；stdio 需 command + args 数组）
- 创建后用 list_mcp_tools + mcp_call_tool 调用
- 多轮追问时用 <ask> 块渲染成可点按钮`,
  tool: `【技术规范·仅你内部参考，不要原样念给用户】
- 创建工具：write_custom_tool
- 必填：id（英文小写唯一）、name、description、code（一段小代码）
- 创建后用 call_custom_tool(toolId, arguments) 调用
- 多轮追问时用 <ask> 块渲染成可点按钮`
}

// 收集历史对话 transcript（供 AI 分析沉淀）
const buildExtensionTranscript = () => {
...
// 构造工具沉淀引导指令：先分析历史对话，再决定直接创建 / 追问用户
const buildExtensionGuideText = (type) => {
  const label = EXTENSION_TYPE_LABEL[type] || '扩展'
  const en = EXTENSION_TYPE_EN[type] || ''
  const transcript = buildExtensionTranscript()
  return `【工具沉淀】请分析下面这段历史对话，判断能否沉淀为一个可复用的「${label}」${en ? `（${en}）` : ''}。

先分析、再动手：
1. 先仔细读历史对话，找出用户反复使用、已验证成功、值得固化成「${label}」的流程/方法/需求。
2. 能明确沉淀出 1 个 → 直接用对应工具创建；信息不全时用 <ask> 一次问 1 个问题补齐。
3. 有多个候选 → 用 <ask>（single）让用户选一个，再做后续。
4. 对话内容不足以沉淀 → 用大白话说明，并用 <ask> 或一句提问问用户想沉淀什么，不要假装分析出来了。
5. 创建成功后，用大白话告诉用户：它叫什么、以后在输入框里怎么调用（给一句可复制的示例）。

${EXTENSION_GUIDE_SPEC[type] || ''}
...
## ROUTING
- Feishu docs/drive/cloud → activate_tools("feishu") → feishu_list_files. NO local search tools.
- WeChat/Feishu send msg/files → activate_tools("wechat"|"feishu") → send_*.
- Local files → find_local_file / list_directory / read_local_file.
- PDF/Word/PPT to mindmap → convert_doc_to_mindmap (background, off-screen, no switch, no overwrite, preserves content). Single: file_path=...; multiple: file_paths=[...]. NEVER read each file with read_local_file then generate_mindmap.
- Knowledge base search → search_knowledge_base / semantic_search.
...
## ASK USER
- When you MUST ask a choice/confirmation, output ONE <ask> JSON block in your FINAL reply: <ask>{"type":"single","options":["A","B"],"question":"question"}</ask>
- type: "single" or "multi". options: 2-8 short labels. question: optional.
- UI ONLY renders clickable buttons from <ask> — plain-text questions won't get buttons. ALWAYS use <ask>.
- Never put <ask> inside intermediate/tool-call text or <plan>/<step-done> blocks; only in final answer.

## QUERY / BATCH
- Prefer query_nodes over repeated search_nodes + query_node_styles.
- For multi-node edits use batch_node_actions; read each tool's description for exact params.
- clear_cloze supports before/after ranges; check tool description.

## CONTEXT
- 【Drag-in file｜path: xxx】→ retrieve_local_file (Q&A) or read_local_file (full text).
- 【Referenced node｜uid=xxx】→ apply directly, don't re-search.
- Pasted content → generate_mindmap.
- memory(action=save) only for stable long-term preferences.

## DOC READING
- Reuse first retrieve_local_file result for same topic; don't repeatedly call with slightly changed keywords.
- Wrong path → find_local_file(keyword=basename) once, use exact returned path.
...
// 记录每条消息的 reasoning-content DOM 引用（用于滚动到底部）
const reasoningRefs = new Map()

const setReasoningRef = (msgId, el) => {
  if (el) {
    reasoningRefs.set(msgId, el)
    // 展开时，下一帧滚动到底部，确保用户看到最新的思考内容
    requestAnimationFrame(() => {
      const pre = el.querySelector('pre')
      if (pre) {
        el.scrollTop = pre.scrollHeight
      }
    })
  } else {
    reasoningRefs.delete(msgId)
  }
}
...
const toggleReasoning = (msg) => {
  ...
  if (msg.reasoningExpanded) {
    // 展开后延迟滚动，等 transition 完成后再滚到底部
    setTimeout(() => {
      const el = reasoningRefs.get(msg.id)
      if (el) {
        const pre = el.querySelector('pre')
        if (pre) el.scrollTop = pre.scrollHeight
      }
    }, 200)
  }
}

// reasoning 内容更新时，如果当前是展开状态，自动滚动到底部
const scrollReasoningToBottom = (msgId) => {
  const el = reasoningRefs.get(msgId)
  if (!el) return
  const pre = el.querySelector('pre')
  if (pre) {
    // 只有当用户已经在底部附近时才自动滚动，避免打断用户阅读
...
// ========== 文本工具调用解析与执行（多模态无 tools 模式兜底） ==========

// 从 AI 回复文本中解析 JSON 格式的工具调用
// 支持 ```json_tool_calls 代码块 和 ```json 代码块
const parseTextToolCalls = (text) => {
  if (!text) return null
  // 优先匹配 json_tool_calls 标记的代码块
  let match = text.match(/```json_tool_calls\s*\n([\s\S]*?)\n```/i)
  if (!match) {
    // 兜底：匹配普通 json 代码块
    match = text.match(/```json\s*\n([\s\S]*?)\n```/i)
  }
  if (!match) {
    // 兜底：匹配第一个看起来像工具调用数组的 JSON（以 [{ 开头）
    const jsonMatch = text.match(/\[\s*\{[\s\S]*?"name"\s*:\s*"[^"]+"[\s\S]*?\}\s*\]/)
    if (jsonMatch) match = [jsonMatch[0], jsonMatch[0]]
  }
  ...
  try {
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      // 验证格式：每项必须有 name 字段
      const valid = parsed.filter(item => item && typeof item.name === 'string')
      if (valid.length > 0) {
...
// 按工具名执行工具（内部构造 toolCall 对象调用 handleToolCall）
const executeToolByName = async (toolName, args = {}) => {
  ...
  return await handleToolCall(toolCall, props.mindMap, null, taskExtraHandlers)
}
...
const sendMessage = async (overrideText = null) => {
  ...
  let isExtensionGuide = false // 是否为工具沉淀引导消息（跳过改写，避免破坏引导指令）
  ...
  // 工具沉淀引导：先分析历史对话（消息以原子胶囊展示，不显示完整指令）
  if (pendingExtensionGuide) {
    const { type } = pendingExtensionGuide
    pendingExtensionGuide = null
    extensionGuideType = type
    isExtensionGuide = true
    inputText.value = ''
    if (textareaRef.value) textareaRef.value.style.height = 'auto'
    text = buildExtensionGuideText(type)
  }
  ...
  // 工具沉淀引导：消息以原子胶囊（aiAction chip）展示，完整指令走 fullContent，不显示大段 prompt
  const displayContent = extensionGuideType
    ? `根据当前对话沉淀「${EXTENSION_TYPE_LABEL[extensionGuideType] || '扩展'}」`
    : text
  ...
  if (extensionGuideType) {
    userMsg.aiAction = '工具沉淀 · ' + (EXTENSION_TYPE_LABEL[extensionGuideType] || '扩展')
    extensionGuideType = null // 用完即清理，避免影响后续普通消息
  }
  ...
  // ==================== 智能多模态路由 ====================
  // 自动判断文件类型，选择最优处理路径，多级降级兜底
  // 优先级：独立多模态配置档 → 主模型配置（自动探测是否支持多模态） → 本地OCR/解析
  ...
  const visionFiles = [...imageFiles, ...docFiles]

  let visionOverride = null
  let visionModelName = ''
  let ocrContext = ''
  let smartRouteResult = null
  let shouldOmitToolsForVision = false

  if (visionFiles.length > 0 && window.electronAPI) {
    // 收集所有可能的多模态配置源
    const possibleConfigs = []

    // 1) 优先：用户单独配置的多模态配置档
    if (window.electronAPI.getVisionConfig) {
      try {
    ...
    // 2) 其次：尝试用主模型配置（很多服务的主模型也支持多模态，不用用户额外配）
    if (possibleConfigs.length === 0 && window.electronAPI.getAIConfig) {
      try {
        const ac = await window.electronAPI.getAIConfig()
        if (ac && ac.baseURL && ac.model && ac.profiles) {
          const activeProfile = ac.profiles.find(p => p.id === ac.activeProfileId)
          if (activeProfile) {
            // 先查已知纯文本模型（硬编码列表，最准确）
            const knownTextOnly = isKnownTextOnlyModel(ac.model)
            if (!knownTextOnly) {
              // 再查能力发现缓存（动态学习的结果）
              const cap = getModelCapability(ac.baseURL, ac.model)
              const probeCached = isVisionSupportedCached(ac.baseURL, ac.model)
              // 已知不支持 → 不浪费请求
              const knownNotSupported =
                (cap && cap.vision === false) ||
                probeCached === false
              if (!knownNotSupported) {
                possibleConfigs.push({
                  source: 'base_profile',
                  config: {
                    baseURL: ac.baseURL,
                    profileId: ac.activeProfileId || '',
                    model: ac.model,
                    autoComplete: activeProfile.autoComplete !== false,
                    filesURL: activeProfile.filesURL || ''
                  }
                })
              }
            }
    ...
    if (possibleConfigs.length > 0) {
      // 使用第一个可用配置
      const chosen = possibleConfigs[0]
      visionOverride = { ...chosen.config, _source: chosen.source }
      visionModelName = chosen.config.model

      // 用智能路由决策最优策略
      const enrichedFiles = visionFiles.map(f => ({
        path: f.path,
        fileName: f.fileName,
        ext: f.ext,
        size: f.size || 0
      ...
      const strategy = decideStrategy(enrichedFiles, chosen.config)
      shouldOmitToolsForVision = strategy.shouldOmitTools

      // 如果策略建议直接走 OCR（比如已知 files API 成功率极低），提前做 OCR
      if (strategy.recommendedPath === 'ocr' && imageFiles.length > 0) {
        ocrContext = await ocrImageFiles(imageFiles)
        addLog('ocr', `智能路由：已自动选择本地 OCR（该服务多模态历史表现不佳），识别 ${imageFiles.length} 张图片`, {
          images: imageFiles.map(f => f.path),
          reason: strategy.reason
        }, currentConversation.value?.id)
        emit('log-updated')
        visionOverride = null // 清掉，走纯文本 OCR 路径
      } else {
        addLog('vision_route', `智能多模态路由：${strategy.reason}`, {
          strategy: strategy.recommendedPath,
          provider: strategy.provider,
          hasImage: strategy.hasImage,
          hasPdf: strategy.hasPdf,
          hasOffice: strategy.hasOffice,
          omitTools: strategy.shouldOmitTools,
          configSource: chosen.source
        }, currentConversation.value?.id)
        emit('log-updated')
      }
    } else {
      ...
      if (imageFiles.length > 0) {
        ocrContext = await ocrImageFiles(imageFiles)
        addLog('ocr', `未配置多模态服务，已后台本地 OCR 识别 ${imageFiles.length} 张图片`, {
          images: imageFiles.map(f => f.path)
        }, currentConversation.value?.id)
        emit('log-updated')
      }
  ...
  if (files.length > 0) {
    ...
    if (imageFiles.length > 0 && window.electronAPI?.fs?.readBinary) {
      const imagesForMsg = []
      // 超大图片不生成 base64 缩略图（避免数 MB base64 占用内存/持久化空间），消息里仅以文件胶囊展示
      const MAX_THUMB_B64 = 2 * 1024 * 1024 // 2MB base64 字符（约 1.5MB 原图）
      for (const f of imageFiles.slice(0, 9)) {
        try {
          const r = await window.electronAPI.fs.readBinary(f.path)
          if (r && r.success && r.base64 && r.base64.length <= MAX_THUMB_B64) {
  ...
  try {
    ...
    if (text && text.trim() && !isExtensionGuide) {
      try {
    ...
    // AI 回复消息占位（reactive 包装：后续 aiMsg.plan/content/toolCalls 的赋值才能触发响应式，
    // 否则原始对象引用修改不会驱动 plan 悬浮窗与流式内容更新）
    ...
    // 方案B：多模态无 tools 模式下，把核心工具描述注入 system prompt
    // 要求 AI 用 JSON 格式输出工具调用，客户端解析后执行
    let visionTextTools = false
    if (visionOverride && shouldOmitToolsForVision) {
      // 生成工具简要描述（精简版，避免占用过多 token）
      const toolDescs = coreTools.map(t => {
...
## TEXT-BASED TOOL USE (CRITICAL — READ CAREFULLY)
This model does NOT support native function_calls. You MUST output tool calls as a JSON code block — DO NOT just say "I will call X tool" or "I will activate X tool". That does NOT work.

Available tools:
${toolDescs}

### How to call tools
Output a JSON code block with EXACTLY this format. Put it FIRST in your reply:
\`\`\`json_tool_calls
[
  {
    "name": "convert_doc_to_mindmap",
    "arguments": { "file_paths": ["path/to/file1.pdf", "path/to/file2.pdf"] }
  }
]
\`\`\`

### STRICT RULES (violations cause failure)
1. Output the \`\`\`json_tool_calls block FIRST, before any explanation text.
2. The JSON must be valid: proper quotes, commas, brackets. No trailing commas.
3. Use the EXACT tool name from the list above. Do NOT make up tool names.
4. For document-to-mindmap requests, ALWAYS use convert_doc_to_mindmap with file_paths array.
5. You can include multiple tool calls in one array.
6. After you output the JSON block, the system will execute the tools and send you the results. Then you continue.
7. If no tools are needed, answer normally without any JSON block.

### WRONG WAY (do NOT do this):
"I will call the activate_tools tool to..."

### RIGHT WAY:
\`\`\`json_tool_calls
[{"name":"convert_doc_to_mindmap","arguments":{"file_paths":["C:/file.pdf"]}}]
\`\`\`
`
      systemPrompt += toolUseInstruction
      visionTextTools = true
    }
    ...
    if (visionOverride && (imageFiles.length > 0 || docFiles.length > 0)) {
      ...
      const uploadOne = async (f, mimeType) => {
        try {
          ...
          // 上传失败降级 base64 直发；但过大图片的 base64（数 MB）会撑爆上下文并卡死渲染进程，
          // 超过阈值改为标记走本地 OCR 文本兜底，不再把原始 base64 塞进消息。
          const MAX_DIRECT_B64 = 1024 * 1024 // 1MB base64 字符（约 750KB 原图）
          if (r.base64 && r.base64.length > MAX_DIRECT_B64) {
            addLog('tool_call', `工具: files API 上传 [files-api]\n图片过大（${Math.round(r.base64.length / 1024)}KB base64），跳过 base64 直发，改用 OCR 文本`, {}, currentConversation.value?.id)
            emit('log-updated')
            return { _fallbackOcr: { path: f.path, fileName: f.fileName, ext: f.ext } }
          }
          return { _fallbackBase64: r.base64 }
        } catch (err) { return null }
      }
      // 图片：files API 上传，失败降级 base64 直发（过大图片走 OCR 文本兜底）
      let filesApiCount = 0
      const ocrFallbackImgs = []
      for (const f of imageFiles.slice(0, 4)) {
        ...
        if (!part) continue
        if (part._fallbackOcr) {
          ocrFallbackImgs.push(part._fallbackOcr)
          continue
        }
      ...
      // 过大图片上传失败后走本地 OCR，结果以文本追加（避免把数 MB base64 塞进消息卡死渲染进程）
      let ocrFallbackText = ''
      if (ocrFallbackImgs.length) {
        try {
          ocrFallbackText = await ocrImageFiles(ocrFallbackImgs.map(p => ({ path: p.path, fileName: p.fileName, ext: p.ext })))
        } catch (e) { ocrFallbackText = '' }
      }
      if (parts.length > 0 || ocrFallbackText) {
        ...
        const fallbackNote = ocrFallbackText ? `\n\n（以下为部分图片的本地 OCR 识别结果，供回答参考）\n\n${ocrFallbackText}` : ''
        const textPart = `${finalUserText}\n\n（本条消息附带 ${parts.length} 个文件，${viaText}。请直接查看文件/图片内容回答，无需再调用 read_local_file / retrieve_local_file 重复读取；文件路径见【拖入文件】标记，需要对文件执行其他操作时再调用工具）${fallbackNote}`
    ...
    let _visionFallbackToMainTried = false // 是否已尝试从多模态切回主模型
    let contextOverflowRetried = false
    let _triedStrategies = []       // 已尝试过的策略 ID（用于避免重复尝试 oneShot 策略）
    let _pendingStrategyId = null   // 当前正在重试的策略 ID（成功后记录）
    let _filesApiFailed = false     // files API 是否失败过
    const attemptChat = async (msgs, override, omitTools = false) => {
      ...
      },
      {
        ...
        onStepDone: (n) => {
          ...
          // 忽略超出计划范围的 step 编号（AI 可能输出多余/重新编号的 step-done，导致进度错乱如 10/4）
          if (!Number.isFinite(n) || n < 1 || n > aiMsg.plan.steps.length) return
        ...
        onChunk: (chunk) => {
          ...
          fullResponse += chunk
          // rAF 节流：每帧只更新一次 content + 滚动，降低 markdown 全量重渲染频率
          if (!chunkRaf) {
        ...
        onBeforeRetry: (hits) => {
          if (!isCurrentRun(runToken)) return
          // 取消待执行的 chunk 渲染，避免旧 fullResponse 覆盖重答内容
        ...
        onToolCall: async (toolCall) => {
          ...
          try {
            ...
            // ask 工具：把 <ask> 块追加到 AI 消息末尾，前端会解析成可点击按钮
            if (result && result._isAsk && result._askBlock) {
              fullResponse += `\n\n${result._askBlock}`
              aiMsg.content = fullResponse
            }
            ...
            if (result && result.filePath && !result.notNewFile) {
              if (result.externalFile) {
                ...
                if (result.switchFile) {
                  // 跳转前记录当前文件，用于返回按钮
                  if (props.currentFilePath && props.currentFilePath !== result.filePath) {
                    previousFileInfo.value = {
                      filePath: props.currentFilePath,
                      fileName: props.currentFileName || ''
                    }
                  }
                  await waitForFileSwitch(result.filePath)
                  // 文件切换完成后，居中到根节点，确保用户看到完整导图
                  try {
                    const renderer = props.mindMap?.renderer
                    if (renderer && renderer.root && typeof renderer.moveNodeToCenter === 'function') {
                      // 延迟一点，等渲染完成
                      setTimeout(() => {
                        try {
                          renderer.moveNodeToCenter(renderer.root)
                          props.mindMap?.render?.()
                        } catch (_) {}
                      }, 300)
                    }
        ...
        onDone: async () => {
          ...
          if (!isCurrentRun(runToken)) return
          // flush 最后一次 chunk 更新：确保最终内容完整落屏（rAF 可能尚未执行）
          if (chunkRaf) {
            cancelAnimationFrame(chunkRaf)
            chunkRaf = null
            aiMsg.content = fullResponse
          }
          // 多模态请求成功 → 记录到记忆库（用于自学习，越用越准）
          if (visionOverride && (imageFiles.length > 0 || docFiles.length > 0)) {
            recordVisionSuccess(visionOverride.baseURL, visionOverride.model, !visionNoToolsTried)
          }
          // 策略自愈成功 → 记录策略成功（提升该策略优先级）
          if (_pendingStrategyId) {
            recordStrategyResult(
              visionOverride?.baseURL || currentBaseURL.value,
              visionOverride?.model || currentModelName.value,
              _pendingStrategyId,
              true
            )
          ...
          // 方案B：多模态文本工具调用模式（无原生 tools 支持时）
          // 检测 AI 输出中的 JSON 工具调用块，解析并执行，执行完把结果送回 AI 继续推理
          const respText = (fullResponse || '').trim()
          if (visionTextTools && !_visionFallbackToMainTried && !stopRequested.value && isCurrentRun(runToken)) {
            const textToolCalls = parseTextToolCalls(respText)
            if (textToolCalls && textToolCalls.length > 0) {
              addLog('smart_fallback', `🤖 智能自愈：检测到文本格式工具调用（${textToolCalls.map(t => t.name).join('、')}），正在执行`, {
              ...
              for (const tc of textToolCalls) {
                ...
                try {
                  ...
                  tcEntry.status = 'error'
                  tcEntry.errorBrief = firstN(e.message, 200)
                  toolResults.push({ name: toolName, error: e.message })
                  addLog('tool_error', `工具: ${toolName}\n错误: ${e.message}`, {
                    toolName,
                    error: e.message
                  }, currentConversation.value?.id)
                  emit('log-updated')
              ...
              // 把工具结果作为 user 消息送回 AI，继续推理（仍然用多模态模型，因为文件可能还在上下文中）
              const resultMsg = toolResults.map(r => {
                const content = r.error
                  ? `Error: ${r.error}`
                  : JSON.stringify(r.result && r.result.success !== undefined ? r.result : { success: true, data: r.result })
                return `Tool ${r.name} result:\n${content}`
              }).join('\n\n')
              const continuedMsgs = [
                ...messagesToSend,
                { role: 'assistant', content: respText },
                { role: 'user', content: `Here are the tool execution results:\n\n${resultMsg}\n\nPlease continue with the task based on these results. If you need more tools, output another JSON tool call block. If the task is complete, give the final answer.` }
              ]
              runStats.toolCalls += textToolCalls.length
              aiStatus.value = 'thinking'
              emit('tool-call-status', 'thinking')
              // 下一轮用同样的多模态无 tools 模式继续（保持文件上下文）
              attemptChat(continuedMsgs, visionOverride, true)
              return
            }
          }
          // 方案C兜底：多模态无 tools 且 AI 没输出 JSON 工具调用，但提到了工具
          // → 自动切回主模型（带完整 tools）继续执行任务
          const visionNoTools = visionOverride && (shouldOmitToolsForVision || visionNoToolsTried)
          const noToolsCalled = runStats.toolCalls === 0 && aiMsg.toolCalls.length === 0
          const mentionsTools = visionNoTools && noToolsCalled && respText &&
            /(调用|使用|激活|activate_tools|convert_doc|generate_mindmap|search_|read_|find_).*(工具|tool)/i.test(respText)
          if (mentionsTools && !_visionFallbackToMainTried && !stopRequested.value && isCurrentRun(runToken)) {
            _visionFallbackToMainTried = true
            addLog('smart_fallback', '🤖 智能自愈：多模态模型不支持工具调用，已切换到主模型继续执行任务', {
              fromModel: visionModelName,
              toModel: currentModelName.value
            }, currentConversation.value?.id)
            emit('log-updated')
            aiStatus.value = 'thinking'
            emit('tool-call-status', 'thinking')
        ...
        onError: async (error) => {
          ...
          // ==================== 智能错误策略引擎 ====================
          // 语义化分析错误 → 从策略库匹配最佳修复方案 → 自动执行 → 记录结果 → 越用越准
          // 支持：多模态降级、限流重试、鉴权报错、上下文超限压缩、服务端错误重试等
          const strategyMem = getStrategyMemory(
            visionOverride?.baseURL || currentBaseURL.value,
            visionOverride?.model || currentModelName.value
          )

          const strategyContext = {
            isVisionRequest: !!visionOverride,
            hasImages: imageFiles.length > 0,
            hasDocs: docFiles.length > 0,
            triedNoTools: visionNoToolsTried,
            triedOcr: ocrFallbackTried,
            triedStrategies: _triedStrategies,
            failedOnFilesApi: _filesApiFailed,
            isImage: imageFiles.length > 0,
            provider: visionOverride?._detectedProvider || '',
            model: visionOverride?.model || currentModelName.value
          }
          ...
          // 记录策略尝试（后面成功了再记成功）
          if (nextAction.strategyId) {
            _triedStrategies.push(nextAction.strategyId)
          }

          // ---------- 策略：去掉 tools 重试 ----------
          if (nextAction.action === 'retry_no_tools' && visionOverride) {
            visionNoToolsTried = true
            recordVisionFailure(visionOverride.baseURL, visionOverride.model, true)
            addLog('smart_fallback', `🤖 智能自愈：${nextAction.reason}`, {
              strategy: nextAction.strategyId,
              model: visionModelName,
              matchScore: nextAction.topStrategy?.matchScore
            }, currentConversation.value?.id)
            emit('log-updated')
            aiMsg.content = ''
            aiMsg.toolCalls.forEach(tc => { tc.status = 'stopped' })
            ...
            // 成功后在 onDone 里记录策略成功
            _pendingStrategyId = nextAction.strategyId
            return
          }

          // ---------- 策略：降级本地 OCR ----------
          if (nextAction.action === 'retry_ocr' && imageFiles.length > 0) {
            ocrFallbackTried = true

            // 能力发现：标记该模型不支持视觉（下次直接跳过）
            if (visionOverride) {
              updateModelCapability(visionOverride.baseURL, visionOverride.model, {
                vision: false,
                visionProbed: true
              })
              // 如果是主模型自动探测失败，也同步到探测缓存
              if (visionOverride._source === 'base_profile') {
                setVisionProbeResult(visionOverride.baseURL, visionOverride.model, false)
              }
            }
              ...
              try {
                ...
                if (nextAction.strategyId) {
                  recordStrategyResult(
                    visionOverride?.baseURL || currentBaseURL.value,
                    visionOverride?.model || currentModelName.value,
                    nextAction.strategyId,
                    true
                  )
                }
                // OCR 降级后用主模型（不再用多模态配置）
          ...
          // ---------- 策略：限流延迟重试 ----------
          if (nextAction.action === 'retry_after_delay') {
            const delayMs = 2000 + Math.random() * 2000 // 2-4秒随机退避
            addLog('smart_fallback', `🤖 智能自愈：${nextAction.reason}，${Math.round(delayMs/1000)}秒后重试`, {
              strategy: nextAction.strategyId,
              delayMs
            }, currentConversation.value?.id)
            emit('log-updated')
            ...
            setTimeout(() => {
              if (!isCurrentRun(runToken)) return
              attemptChat(messagesToSend, visionOverride, visionNoToolsTried)
            }, delayMs)
            _pendingStrategyId = nextAction.strategyId
            return
          }
          ...
          // ---------- 策略：上下文超限压缩 ----------
          if (nextAction.action === 'retry_shorter_context') {
            addLog('smart_fallback', `🤖 智能自愈：${nextAction.reason}，正在压缩上下文...`, {
              strategy: nextAction.strategyId
            }, currentConversation.value?.id)
            emit('log-updated')
            // 走已有的自动压缩逻辑
            if (messagesToSend.length > 4) {
              try {
                const rebuilt = await autoCompressAndRebuild(messagesToSend, newSummary)
                aiMsg.content = ''
                aiMsg.toolCalls.forEach(tc => { tc.status = 'stopped' })
                aiMsg.toolCalls = []
                fullResponse = ''
                aiStatus.value = 'thinking'
                emit('tool-call-status', 'thinking')
                addLog('compress', `自动压缩完成，正在重试...`, {}, currentConversation.value?.id)
                emit('log-updated')
                await attemptChat(rebuilt, visionOverride)
                _pendingStrategyId = nextAction.strategyId
                return
              } catch (compressErr) {
                console.error('自动压缩重试失败:', compressErr)
          ...
          // 任一情况下若直接结束回调，aiStatus 会永远停在 thinking，界面表现为"一直转圈不回复"。
          {
            ...
            emit('tool-call-status', 'error')
            aiService.resetAbort()
            addLog('error', `${error.message || 'AI 服务异常'}\n${nextAction.reason ? '诊断：' + nextAction.reason : ''}`, {
              model: currentModelName.value,
              strategy: nextAction.strategyId
            }, currentConversation.value?.id)
            emit('log-updated')
...
`
  ...
  if (isMulti) {
   ...
   - 记忆口诀（memoryKey）：4~10 字，必须从这些节点里真实出现的核心关键词提炼，让人一看口诀就能回忆起原文讲了什么。可用「数字+核心概念」（如"三大来源""五步闭环"）、首字串联，或自然顺口的谐音/联想——但都要建立在原文关键词之上；禁止生造与内容无关的顺口溜或空泛套话。若内容本身不适合口诀化，就用一个凝练的核心短语概括，不要硬凑押韵。
   - 背诵改写（reciteText）：把这几个节点的核心知识点整合成一段便于背诵的精炼表述，忠于原文含义、保留全部关键知识点，语句通顺自然、有逻辑衔接，100 字以内；不要只是把词语堆在一起，也不要遗漏或篡改原意。
...
2. ${commonRule}`
   ...
   - 记忆口诀（memoryKey）：4~10 字，必须从子节点内容里真实出现的核心关键词提炼，让人一看就能回忆起这些子节点讲了什么（如子节点讲三个特点可叫"三大特点"，讲四个阶段可叫"四阶段流程"）。可用数字概括、首字串联或自然顺口的谐音/联想，但都要建立在子节点关键词之上；禁止生造无关顺口溜或空泛套话。若不适合口诀化，就用一个凝练的核心短语概括。
   - 背诵改写（reciteText）：把该节点「下一层子节点」的核心知识点整合成一段便于背诵的精炼表述，忠于原文、保留关键知识点，语句通顺自然、有逻辑衔接，100 字以内；不要只堆词，也不要遗漏或篡改原意。
   - 简要解释（explanation）：用一句话概括下一层子节点之间的逻辑关系与记忆要点，40 字以内。
   更深层的子节点内容仅作为理解上下文，不需要逐一展开。
2. ${commonRule}`
  } else {
    ruleContent = `1. 生成三部分内容：
   - 记忆口诀（memoryKey）：2~10 字，直接提炼原文中最核心的概念词，让人一看就对应上原文（如原文讲"客观实在性"就叫"客观实在"，讲"对立统一"就叫"对立统一"）；也可用自然顺口的首字串联/谐音联想，但必须建立在原文关键词之上，禁止生造与内容无关的顺口溜或空泛套话。
   - 背诵改写（reciteText）：将原节点内容改写为便于背诵的精炼表述，忠于原文、语句通顺自然、保留全部关键知识点，60 字以内。如果原节点文字 ≤ 12 个字，直接保留原文字。
...
改写规则（严格遵守）：
${ruleContent}
3. memoryTip：针对本次改写涉及的最高层主题输出一个整体记忆技巧，简短（30字内）、自然、好记，且必须建立在本次内容的真实关键词之上（帮助串联记忆），不要泛泛而谈；如果编不出自然好记的谐音，就直接给语义归纳或首字串联，不要强行谐音。
...
// 清洗改写文本：把 AI 可能返回的 HTML 标签 / markdown / 代码围栏全部剥成纯文本，
// 再去掉多余的标点前缀（冒号、空格、破折号、逗号等）。
// 否则这些标记会被 toReciteGeneralizationHtml 转义后原样显示在节点里（用户看到 <span>、**、``` 等）。
const cleanRewriteText = (t) => {
  ...
  s = s.replace(/`+/g, '')
  // 2) HTML：<br> 转空格，其余标签整体删除
  s = s.replace(/<\s*br\s*\/?\s*>/gi, ' ')
  s = s.replace(/<\/?\s*[a-z!?][^>]*>/gi, '')
  // 3) markdown 图片/链接 → 只保留可读文本
  s = s.replace(/!\[([^\]]*)\]\([^)]*\)/g, '$1')
  s = s.replace(/\[([^\]]+)\]\([^)]*\)/g, '$1')
  // 4) markdown 标题/引用/列表标记（按行处理）
  s = s
  ...
  s = s.replace(/(\*|_)(.*?)\1/g, '$2')
  s = s.replace(/~~(.*?)~~/g, '$1')
  // 6) HTML 实体：丢弃残留的转义尖括号（绝不能让 <> 进入节点），其余还原为可读字符
  s = s
    .replace(/&lt;|&gt;/gi, '')
    .replace(/&nbsp;/gi, ' ')
    .replace(/&quot;/gi, '"')
...
const aiCloze = async (nodesOrNode, opts = {}) => {
  ...
  if (!viaTool) {
    try {
      await ElMessageBox.confirm(
        '「仅兜底挖空」直接命中内置正则/结构规则，速度快、不调用 AI；\n「AI 介入挖空」会在规则基础上用 AI 补充和精修，更全面但较慢。',
        '选择挖空方式',
        {
          confirmButtonText: 'AI 介入挖空',
          cancelButtonText: '仅兜底挖空',
          distinguishCancelAndClose: true,
          type: 'info'
        }
      )
      aiEnabled = true // 点击「AI 介入挖空」
    } catch (action) {
      if (action === 'cancel') {
        aiEnabled = false // 点击「仅兜底挖空」
      } else {
        // 右上角关闭（close）或 Esc：取消本次操作，不做任何修改
        return '已取消挖空'
      }
    }
  ...
  try {
    ...
    const progressCb = (progress) => {
      ...
      if (progress && typeof progress === 'object' && progress.phase === 'fallback_done') {
        if (aiEnabled) {
          ...
          // 仅兜底挖空：规则阶段即终点
          progressText = progress.message || '规则挖空完成'
          if (clozeToolCalls[0]) {
            clozeToolCalls[0].summary = '规则挖空完成'
          }
        }
    ...
    const clozeRes = isFullMap
      ? await smartClozeFullMap('smart', progressCb, { aiEnabled })
      : await smartClozeNodes(nodes, 'smart', progressCb, { aiEnabled })
    ...
    const isFallbackOnly = !!(clozeRes && typeof clozeRes === 'object' && clozeRes.aiEnabled === false)
    ...
    let clozeSourceText = ''
    if (isFallbackOnly) {
      // 仅兜底挖空：只跑了正则/结构规则，未调用 AI
      clozeSourceText = `\n\n规则挖空 ${fallbackCount} 个关键词（未调用 AI）。`
    } else if (isTwoPhase) {
      // 三步流程的详细统计：机器挖空 → AI 补挖 → AI 精修
      clozeSourceText = `\n\n⚡ 机器挖空 ${fallbackCount} 个，AI 补挖 ${aiCount} 个，精修补充 ${reviewAdded} 个，移除 ${reviewRemoved} 个。`
    } else if (fallbackCount > 0) {
      clozeSourceText = `\n\nAI 挖空 ${aiCount} 个，本地兜底挖空 ${fallbackCount} 个。`
    }

    aiMsg.content = `已完成 ${nodeCount} 的${isFallbackOnly ? '规则挖空' : '智能挖空'}，共挖空 ${count} 个关键词。${clozeSourceText}\n\n挖空内容已隐藏（半透明紫底+紫下划线），可通过画布右键菜单「显示/隐藏挖空」切换显隐状态。可通过 Ctrl+Z 撤销。`
    aiStatus.value = 'done'
    emit('tool-call-status', 'done')
    ...
    if (!viaTool) {
      let logDetail = ''
      if (isFallbackOnly) {
        logDetail = `仅兜底模式（未调用 AI）\n规则挖空: ${fallbackCount} 个`
      } else if (isTwoPhase) {
...
// 合并多个文档的 Markdown 大纲为一个分级大纲：根 = 整体标题，各文档作为一级分支，其层级整体下移
const mergeMultiDocMarkdown = (markdownParts, rootTitle) => {
  // 根标题转义：防止文件名含 <>& 等字符被当作 HTML/Markdown 注入
  const safeRoot = String(rootTitle || '')
    .replace(/[<>&]/g, c => ({ '<': '&lt;', '>': '&gt;', '&': '&amp;' }[c]))
    .replace(/[\r\n]/g, '')
  const root = `# ${safeRoot}`
...
const convertDocToMindmap = async (filePath, fileName, options = {}) => {
  const viaTool = !!options?.viaTool
  // 兼容单文件（字符串）与多文件（数组）：多文件时逐个解析后整合成一个导图
  const filePaths = Array.isArray(filePath)
    ? filePath.map(p => String(p || '').trim()).filter(Boolean)
    : (String(filePath || '').trim() ? [String(filePath).trim()] : [])
  if (!filePaths.length) return viaTool ? '请提供 file_path（单个文档）或 file_paths（多个文档数组）' : undefined

  const base = String(fileName || filePaths[0].split(/[\\/]/).pop() || '文档').replace(/\.[^.]+$/, '')

  // 独立 aiService 实例：文档转换在后台独立通道运行，不占用、不阻塞主对话的 aiService，实现「转换中可并行对话」。
  const svc = createAIService()

  // 工具调用入口时，主对话循环已持有 aiStatus（'calling'）。此函数一旦再检测/修改 aiStatus，会「自锁 + 静默假成功」。
  // 因此 viaTool 走 backgroundRunning 标记，绝不触碰 aiStatus / emit('tool-call-status') / 主消息列表。
  let aiMsg = null
  if (viaTool) {
    if (backgroundRunning.has('convert')) {
      return '已有文档转换任务进行中，请等待其完成后再试'
    }
    backgroundRunning.add('convert')
    addLog('tool_call', `工具: AI转换为思维导图\n开始: 正在后台转换「${base}」...`, { toolName: 'convert_doc_to_mindmap' }, currentConversation.value?.id)
    emit('log-updated')
    // 占位消息对象：仅本地跟踪进度与最终返回值，不插入主对话消息列表（主循环已自行渲染工具条目）
    aiMsg = { id: genMsgId(), role: 'assistant', content: '', toolCalls: [{ name: 'convert_doc_to_mindmap', displayName: 'AI 转换为思维导图', status: 'calling' }] }
  } else {
    if (aiStatus.value === 'thinking' || aiStatus.value === 'calling') {
      ElMessage.warning('AI 正在处理中，请稍候...')
      return
  ...
  const setProgress = (text) => {
    if (aiMsg) {
      aiMsg.content = text
      if (aiMsg.toolCalls && aiMsg.toolCalls[0]) {
        aiMsg.toolCalls[0].summary = text
      }
    }
    if (viaTool) {
      addLog('tool_call', `工具: AI转换为思维导图\n进度: ${text}`, { toolName: 'convert_doc_to_mindmap' }, currentConversation.value?.id)
      emit('log-updated')
    } else {
      scrollToBottom()
    }
  ...
  try {
    ...
    // 批次级 files API 失败缓存：同一个转换批次内，只要失败过一次就跳过后续文件的 files API 尝试
    // 避免每个文件都重复尝试→失败→降级，浪费大量时间
    let batchFilesApiFailed = false

    // 解析单个文档 → Markdown 大纲（files API 多模态优先，失败降级本地解析 + AI 分段整理）
    const parseDocToMarkdown = async (fp, fpBase) => {
      const ext0 = String(fp).split('.').pop().toLowerCase()
      ...
      //    批次内已失败过则直接跳过，避免重复浪费时间。
      let visionOverride = null
      if (!batchFilesApiFailed && window.electronAPI && window.electronAPI.getVisionConfig) {
        try {
          const vc = await window.electronAPI.getVisionConfig()
          if (vc && vc.available && vc.baseURL && vc.model) {
    ...
    // 循环解析多个文件（单文件时只循环一次，行为与原逻辑一致）
    const markdownParts = []
    for (let idx = 0; idx < filePaths.length; idx++) {
      const fp = filePaths[idx]
      const fpBase = fp.split(/[\\/]/).pop().replace(/\.[^.]+$/, '')
      setProgress(filePaths.length > 1 ? `正在解析「${fpBase}」（${idx + 1}/${filePaths.length}）...` : '正在读取文档内容...')
      const md = await parseDocToMarkdown(fp, fpBase)
      if (md) markdownParts.push({ base: fpBase, md })
    }
    if (!markdownParts.length) throw new Error('AI 未返回有效的大纲内容')

    // 整合 markdown：单文件直接用；多文件合并为一个分级大纲
    let markdown
    if (markdownParts.length === 1) {
      markdown = markdownParts[0].md
    } else {
      markdown = mergeMultiDocMarkdown(markdownParts, base)
    }
...
const extraHandlers = {
  ...
  convertDocToMindmap: async (args = {}) => {
    const filePaths = Array.isArray(args.file_paths) && args.file_paths.length > 0
      ? args.file_paths.map(p => String(p || '').trim()).filter(Boolean)
      : (String(args.file_path || args.filePath || '').trim() ? [String(args.file_path || args.filePath).trim()] : [])
    if (!filePaths.length) return '请提供 file_path（单个文档）或 file_paths（多个文档数组）'
    const fileName = String(args.file_name || args.fileName || '').trim() || filePaths[0].split(/[\\/]/).pop()
    return convertDocToMindmap(filePaths, fileName, { viaTool: true })
  },
  addToReview: (uid) => {
    try {
...
.chat-panel {
  ...
  position: relative;
  /* 建立独立层叠上下文：悬浮的 Todo/计划面板只在 AI 助手面板内部层叠，
     不会溢出并与其他功能面板的文字交错重叠 */
  isolation: isolate;
  overflow: hidden;
}
...
.plan-float-panel {
  ...
  overflow-y: auto;
  padding: 6px 10px;
  /* 完全不透明背景 + 独立层叠，确保面板浮在消息文字之上，不会与背后文字重合 */
  background: #ffffff;
...
.history-item.is-global .history-item-title {
  color: var(--apple-blue);
}
