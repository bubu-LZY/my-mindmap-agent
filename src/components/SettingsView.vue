<template>
  <div ref="settingsViewRef" class="settings-view" @scroll="onSettingsScroll">
    ...
    <div id="sec-safety" class="settings-section">
      ...
      <!-- 永久记忆（用户手写指令） -->
      <div class="safety-block">
        <div class="safety-block-title">永久记忆</div>
        <p class="safety-block-desc">
          你手写的永久指令，AI 会严格遵守。与下方「AI 长期记忆」（AI 自动积累）相互独立。
        </p>
        <textarea
          v-model="permanentMemory"
          class="memory-textarea"
          placeholder="例如：生成思维导图时，按「特点类 / 关系类」组织知识结构"
          rows="4"
        ></textarea>
        <el-button size="small" type="primary" class="memory-save-btn" @click="savePermanentMemory">
          保存永久记忆
        </el-button>
      </div>
        ...
        <template v-else>
          <div class="whitelist-list">
            <div v-for="fact in memoryFacts" :key="fact.id" class="whitelist-item">
              ...
              <button class="whitelist-remove" @click="editMemoryFactItem(fact)">编辑</button>
    ...
    <div id="sec-cloud-sync" class="settings-section">
      <h3>云盘同步</h3>
      <div class="cloud-sync-guide">
        <p><b>这是什么？</b>开启后，你每保存一次导图，程序就会自动把「C:\我的mindmap」文件夹里的所有导图（包括里面的子文件夹）同步上传到你的网盘，多一份云端备份，换电脑、误删都不怕丢。</p>
        <p><b>第一次使用，跟着下面三步走：</b></p>
        <p><b>第 1 步 · 下载小工具 rclone（免费）</b><br>点右侧「下载 rclone」按钮打开官网下载页，选 <b>Windows 64 位</b> 下载压缩包；解压后得到一个 <b>rclone.exe</b>，把它放到一个你记得住的位置（比如 D:\rclone\rclone.exe），然后把完整路径填到下面「rclone 路径」里。</p>
        <p><b>第 2 步 · 填网盘账号</b><br>「服务商」选你的网盘，选完地址会自动填好。</p>
        <p style="margin-left: 1em;"><b>坚果云：</b>「账号」填注册邮箱；「密码」<b>不是登录密码</b>，要去坚果云网页 → 右上角头像 → 「账户信息」 → 「安全选项」 → 「第三方应用管理」 → 「添加应用」，把生成的那串密码填到「密码」框。（免费版每月上传 1GB、下载 3GB，足够存导图）</p>
        <p style="margin-left: 1em;"><b>TeraBox 海外版：</b>需注册海外版账号，WebDAV 功能需高级版，免费版不支持。</p>
        <p style="margin-left: 1em;"><b>Yandex Disk：</b>俄罗斯网盘，免费 10GB，需俄区账号。WebDAV 地址已预填，账号密码即登录账号密码。</p>
        <p style="margin-left: 1em;"><b>自定义：</b>手动输入你的 WebDAV 地址、账号、密码即可。</p>
        <p><b>第 3 步 · 指定云端文件夹 + 试一次</b><br>「云端目录」填你网盘里想放这些导图的文件夹名，比如填 <b>/我的导图</b>（意思就是网盘根目录下新建一个「我的导图」文件夹来存）。<br>填完后点「保存配置」，再点「立即同步一次」试一下，看到绿色提示「同步完成」就说明通了；最后打开「启用自动同步」，以后保存导图就会自动同步。</p>
      </div>
      <div class="setting-row">
        <span class="setting-label">服务商</span>
        <el-select v-model="cloudSyncForm.vendor" size="small" style="width: 200px;" @change="onCloudVendorChange">
          <el-option v-for="(v, key) in cloudVendors" :key="key" :label="v.label" :value="key" />
        </el-select>
      </div>
      <div class="setting-row">
        <span class="setting-label">WebDAV 地址</span>
        <el-input v-model="cloudSyncForm.url" class="desk-token-input" placeholder="选「服务商」会自动填好；选「自定义」需手动填" />
      </div>
      <div class="setting-row">
        <span class="setting-label">账号</span>
        <el-input v-model="cloudSyncForm.user" class="desk-token-input" placeholder="网盘账号，坚果云填注册邮箱" />
      </div>
      <div class="setting-row">
      ...
      <p style="font-size: 12px; color: #86868b; margin: -6px 0 12px 0;">
        填法示例：想把导图放到网盘<b>根目录下</b>的「我的导图」文件夹 → 填 <b>/我的导图</b>；想直接放网盘<b>根目录</b>（不建子文件夹）→ 填 <b>/</b> 或留空。
      </p>
      <div class="setting-row">
        <span class="setting-label">rclone 路径</span>
        <el-input v-model="cloudSyncForm.rclonePath" class="desk-token-input" placeholder="rclone.exe 完整路径，如 D:\rclone\rclone.exe" />
        <el-button size="small" @click="selectRclonePath">选择</el-button>
        <a href="#" style="font-size: 13px;" @click.prevent="openExternalLink('https://rclone.org/downloads/')">下载 rclone</a>
      </div>
      <div class="setting-row">
        <span class="setting-label">启用自动同步</span>
        <el-switch v-model="cloudSyncForm.enabled" />
      </div>
      <div class="setting-row">
        <el-button size="small" type="primary" @click="saveCloudSync">保存配置</el-button>
        <el-button size="small" :loading="cloudSyncing" @click="testCloudSync">立即同步一次</el-button>
      </div>
    </div>
...
const editMemoryFactItem = async (fact) => {
  try {
    const { value } = await ElMessageBox.prompt('修改这条记忆的内容：', '编辑长期记忆', {
      confirmButtonText: '保存',
      cancelButtonText: '取消',
      inputValue: fact.content,
      inputValidator: (v) => (v && String(v).trim() ? true : '内容不能为空'),
      closeOnClickModal: false
    })
    const newContent = String(value || '').trim()
    if (!newContent) return
    updateMemoryFact(fact.id, newContent)
    loadMemoryFacts()
    ElMessage.success('已更新该条长期记忆')
  } catch {
    // 取消编辑
  }
}
...
// ========== 永久记忆（用户手写指令，AI 严格遵守） ==========
const permanentMemory = ref('')
const loadPermanentMemory = () => { permanentMemory.value = loadMemory() }
const savePermanentMemory = () => {
  saveMemory(permanentMemory.value)
  ElMessage.success('已保存永久记忆')
}
...
const cloudSyncForm = ref(cloudSyncService.loadConfig())
const cloudSyncing = ref(false)

const onCloudVendorChange = (vendor) => {
  const preset = cloudVendors[vendor]
...
const testCloudSync = async () => {
  ...
  try {
    ...
    if (res && res.success) ElMessage.success('同步完成')
...
// 点击「选择」按钮，弹出文件选择框快速选 rclone.exe
const selectRclonePath = async () => {
  const paths = await window.electronAPI?.dialog?.showOpenDialog?.({
    title: '选择 rclone.exe',
    filters: [{ name: 'rclone 程序', extensions: ['exe'] }]
  })
  if (Array.isArray(paths) && paths.length > 0) {
    cloudSyncForm.value.rclonePath = paths[0]
  }
}
