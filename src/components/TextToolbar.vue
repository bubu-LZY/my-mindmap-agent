<template>
  <Transition name="tb-slide">
    <Teleport to="body" :disabled="positionMode !== 'fixed'">
      ...
      <!-- 清除格式 -->
      <button class="tb-btn" @click="exec('removeFormat')" @mousedown.prevent="saveSelection" title="清除格式">
...
const saveSelection = () => {
  ...
  if (quill) {
    ...
    // 鼠标移出节点框后 Quill 失焦，getSelection() 可能返回 null 或折叠光标；
    // 从 RichText 实例的 lastRange 兜底（selection-change 在失焦前保存了有效选区）
    if (!range || range.length === 0) {
      try {
        const mm = typeof props.getMindMap === 'function' ? props.getMindMap() : null
        const rt = mm && mm.richText
        const fb = rt && (rt.range || rt.lastRange)
        if (fb && fb.length > 0) range = fb
      } catch (e) { /* 忽略 */ }
    }
    if (range) {
      savedQuill = quill
...
const exec = (command) => {
  ...
  if (quill) {
    ...
    switch (command) {
      // 改用 applyQuillFormats：用 mousedown 时保存的显式选区 formatText，
      // 而不是 quill.format（依赖当前选区，鼠标移出节点框后选区丢失会退化成整节点生效）
      case 'bold': applyQuillFormats(quill, { bold: !fmt.bold }); break
      case 'italic': applyQuillFormats(quill, { italic: !fmt.italic }); break
      case 'underline': applyQuillFormats(quill, { underline: !fmt.underline }); break
      case 'strikeThrough': applyQuillFormats(quill, { strike: !fmt.strike }); break
      case 'removeFormat': {
        const saved = (savedQuill === quill && savedQuillRange && savedQuillRange.length > 0) ? savedQuillRange : null
        const range = saved || quill.getSelection()
