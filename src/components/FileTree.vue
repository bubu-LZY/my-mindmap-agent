...
const openFile = async (filePath, name) => {
  ...
  // 图片类文件不支持在应用内打开（程序无图片查看器，主进程 open-file 会误把二进制当文本读成乱码
  // 并打开成空白标签）；引导用户拖入 AI 对话由 AI 识别，保持「后台阅读」。
  const imageExts = ['png', 'jpg', 'jpeg', 'bmp', 'gif', 'webp', 'svg', 'ico', 'tif', 'tiff']
  if (imageExts.includes(ext)) {
    ElMessage.info('图片不支持在应用内打开，请拖入 AI 对话框让 AI 识别图片内容')
    return
  }
