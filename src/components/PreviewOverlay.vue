<template>
  ...
  >
    ...
    >
      ...
      <span v-if="targetKind === 'doc'" class="preview-readonly" title="该类型只能查看，程序无编辑能力">只读</span>
      <div class="preview-actions">
        <button class="preview-btn primary" @click="openFile" :title="actionTitle">
          {{ actionLabel }}
        </button>
    ...
    <!-- 缩放/滚动提示 -->
    <div class="preview-zoom-hint">{{ previewType === 'minimap' ? '滚轮缩放 · 左/右/中键拖动查看' : '滚轮滚动查看内容' }}</div>
  </div>
</template>
...
import { getRefKind, getFileExt, resolveRefAction } from '../utils/refTarget'
...
// 当前预览渲染类型：'minimap'（导图）| 'markdown'（md 文本）| 'text' | 'html' | 'pdf'
const previewType = ref('')
// 目标类型：'mindmap'（可编辑）| 'doc'（只读查看）
const targetKind = ref('doc')

// 头部按钮：导图可编辑，PDF/Word/Excel 等文档只能查看
const actionLabel = computed(() => (targetKind.value === 'mindmap' ? '去编辑' : '去查看'))
const actionTitle = computed(() => (
  targetKind.value === 'mindmap'
    ? '在主界面打开并编辑该思维导图'
    : '在文档查看器中打开（只读）'
))
...
const onWheelCapture = (e) => {
  // 非导图预览（文本/HTML/PDF）让容器原生滚动，阻断事件继续传播到画布缩放
  if (previewType.value !== 'minimap') {
    e.stopPropagation()
  }
}
...
/**
 * 头部按钮：导图 = 在主界面打开并编辑（去编辑）；
 * PDF / Word / Excel / Markdown 等文档 = 在文档查看器只读查看（去查看）。
 * 具体走哪条路由由 App.vue 依据同一套类型规则判定，这里只负责传递原始路径。
 */
const openFile = () => {
...
/**
 * 加载并渲染预览
 * 导图文件（.smm/.json）→ 迷你导图；Markdown → 文本；
 * PDF/Word/Excel/PPT/CSV/纯文本等文档 → 解析后只读查看（程序无编辑能力）
 */
const loadPreview = async () => {
  ...
  const kind = getRefKind(props.filePath)
  // 先按扩展名给个初始动作（避免加载期间按钮文案跳变），读取后再校正
  targetKind.value = kind === 'mindmap' ? 'mindmap' : 'doc'

  try {
    // Markdown：先判断是否可当作导图编辑（存在同名 .smm，或内容本身就是导图大纲），
    // 都不是则只能只读查看 md 本身
    if (kind === 'markdown') {
      ...
      const mdText = typeof result.data === 'string' ? result.data : String(result.data ?? '')
      const act = await resolveRefAction(props.filePath, { mdText })
      targetKind.value = act.action === 'edit' ? 'mindmap' : 'doc'
      loading.value = false
      await nextTick()
      previewType.value = 'markdown'
      renderMarkdownPreview(mdText)
      return
    }
    ...
    if (kind === 'mindmap') {
      ...
      // .json 但不是导图结构（普通 JSON/文本）→ 降级为只读文档查看
      targetKind.value = 'doc'
      await renderDocPreview()
      return
    }

    // 其余文档类型（PDF/Word/Excel/PPT/CSV/纯文本…）→ 只读查看
    targetKind.value = 'doc'
    await renderDocPreview()
  } catch (e) {
...
/**
 * 文档预览：PDF 渲染首页图片，其余走统一解析服务（文本 / HTML 表格）
 */
const renderDocPreview = async () => {
  ...
  try {
    ...
    const { parseDocument } = await import('../services/docParseService')
    const res = await parseDocument(props.filePath)
    loading.value = false
    if (!res || !res.success) {
      error.value = res?.error || '文档解析失败'
      return
    }
    await nextTick()
    if (res.html) {
      ...
      renderTextPreview(res.text)
    }
  } catch (e) {
    loading.value = false
    error.value = e?.message || '文档解析失败'
  }
}

/**
 * PDF 预览：渲染第 1 页为图片（悬浮窗不做全文解析，避免大 PDF 卡住界面）
 */
const renderPdfPreview = async () => {
  destroyMiniMap()
  const r = await window.electronAPI?.fs?.readBinary?.(props.filePath)
  if (!r || !r.success) {
    error.value = r?.error || 'PDF 读取失败'
    previewType.value = ''
    return
  }
  let u8 = null
  if (r.data) {
    u8 = r.data instanceof Uint8Array ? r.data : new Uint8Array(r.data)
  } else if (r.base64) {
  ...
  if (!u8) {
    error.value = 'PDF 读取失败'
    previewType.value = ''
    return
  }
  // 悬浮预览只渲染首页，超大 PDF 不在此解析（避免卡住界面），引导用查看器打开
  const sizeMb = Number(r.size || 0) / 1024 / 1024
  if (sizeMb > 25) {
    error.value = `PDF 过大（约 ${sizeMb.toFixed(1)}MB），请用「去查看」在文档查看器中打开`
    previewType.value = ''
    return
  }
  const { pdfToImages } = await import('../utils/pdfToImage')
  const out = await pdfToImages(u8, { pageStart: 1, pageEnd: 1, maxPages: 1, scale: 1 })
  if (!out || !out.success || !out.pages || !out.pages.length) {
    error.value = out?.error || 'PDF 预览失败'
    previewType.value = ''
    return
  }
  ...
  img.alt = '第 1 页'
  img.src = `data:image/png;base64,${out.pages[0]}`
  wrap.appendChild(img)
  const cap = document.createElement('div')
  cap.className = 'pdf-page-cap'
  cap.textContent = out.totalPages > 1 ? `共 ${out.totalPages} 页 · 首页预览` : '单页预览'
...
/**
 * HTML 预览（docx / xlsx / xls / csv）：消毒后渲染
 */
const renderHtmlPreview = (html) => {
  if (!miniMapContainerRef.value) return
  destroyMiniMap()
  miniMapContainerRef.value.innerHTML = sanitizeSafeHtml(`<div class="doc-preview">${html}</div>`)
}

/**
 * 纯文本预览（pptx / txt / log / xml / html 源码等）：截断行数避免超大 DOM
 */
const renderTextPreview = (text) => {
  ...
  const html = lines
    .map(line => `<div class="md-line">${line ? escapeHtml(line) : '&nbsp;'}</div>`)
    .join('')
...
watch(
  ...
  ([visible]) => {
    if (visible && props.filePath) {
      ...
      // 清理迷你思维导图与上一次的预览内容
      destroyMiniMap()
      if (miniMapContainerRef.value) miniMapContainerRef.value.innerHTML = ''
      previewType.value = ''
    }
  },
  { immediate: true }
...
:deep(.doc-preview p) { margin: 0 0 6px; }
:deep(.doc-preview h1) { font-size: 15px; margin: 8px 0 6px; }
:deep(.doc-preview h2) { font-size: 14px; margin: 8px 0 6px; }
:deep(.doc-preview h3) { font-size: 13px; margin: 6px 0 4px; }
:deep(.doc-preview ul),
:deep(.doc-preview ol) { margin: 0 0 8px; padding-left: 20px; }
...
:deep(.doc-table th) { background: rgba(0, 0, 0, 0.04); font-weight: 600; }
:deep(.sheet-name) { font-size: 12px; font-weight: 600; margin: 6px 0 4px; color: #1a1a1a; }
:deep(.sheet-gap) { height: 10px; }
