  ...
  <div class="panel-run-log">
    ...
    <div class="log-header">
      ...
      <div class="log-actions">
        ...
        <button class="log-action-btn" @click="copyAllLogs" title="复制所有日志">
          <svg viewBox="0 0 20 20" fill="none" width="14" height="14">
        ...
        <button class="log-action-btn" @click="copyLastTurnLogs" title="复制最近一次调用日志">
          <svg viewBox="0 0 20 20" fill="none" width="14" height="14">
        ...
        <button class="log-action-btn danger" @click="confirmClear" title="清空日志">
          <svg viewBox="0 0 20 20" fill="none" width="14" height="14">
    ...
    <div class="log-stats" v-if="logs.length > 0">
      ...
      <span class="stat-badge tool" v-if="toolCount > 0">工具 {{ toolCount }}</span>
    ...
    <div class="log-list" ref="logListRef">
      ...
      >
        <div class="log-entry-header">
          ...
          <span class="log-time">{{ formatPanelLogTime(log.timestamp) }}</span>
...
import { loadPanelLogs, clearPanelLogs, formatPanelLogTime } from '../utils/panelLogStore'
...
const toolCount = computed(() => logs.value.filter(l => l.type === 'tool_call' || l.type === 'tool_result' || l.type === 'tool_error').length)
...
const refreshLogs = () => {
  logs.value = loadPanelLogs(props.source)
  scrollToBottom()
}

// 与 AI 对话日志一致：原生 confirm 在 Electron 中可能阻塞，改用 ElMessageBox
const confirmClear = async () => {
  try {
    await ElMessageBox.confirm('确定要清空日志吗？', '清空确认', {
  ...
  clearPanelLogs(props.source)
  logs.value = []
}

// 导出格式与 AI 对话运行日志完全一致
const formatEntry = (log) => {
  const time = formatPanelLogTime(log.timestamp)
...
const copyAllLogs = async () => {
  ...
  const text = header + logs.value.map(formatEntry).join('\n\n---\n\n')
  await copyToClipboard(text, `已复制 ${logs.value.length} 条日志`)
}

// 复制最近一次调用（从最后一条"发送"起）的日志
const copyLastTurnLogs = async () => {
  ...
  const text = turnLogs.map(formatEntry).join('\n\n---\n\n')
  await copyToClipboard(text, `已复制最近一次调用的 ${turnLogs.length} 条日志`)
}
