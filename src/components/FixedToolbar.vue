...
import { toggleAllCloze, setAllClozeHidden, isClozeHiddenAll, setNodesClozeHidden, nodeHasCloze, isUidClozeHidden, hasAnyCloze, setClozeStateForExport, restoreClozeStateForExport } from '../utils/cloze'
...
// 需要挖空显隐选择的导出类型（图片/PDF 等视觉化导出）
const CLOZE_VISUAL_EXPORT_TYPES = new Set(['png', 'pdf', 'pdf-wechat', 'pdf-feishu'])

/**
 * 询问用户导出时挖空的显示方式
 * 返回 true=显示挖空内容（带答案），false=隐藏挖空（不带答案），null=取消导出
 */
const askClozeExportPreference = async () => {
  try {
    const { action } = await ElMessageBox({
      title: '导出挖空设置',
      message: '检测到导图中有挖空内容，请选择导出方式：',
      showCancelButton: true,
      confirmButtonText: '隐藏挖空（练习用）',
      cancelButtonText: '显示答案',
      distinguishCancelAndClose: true,
      type: 'info',
      customStyle: { paddingBottom: '10px' }
    })
    // confirm = 隐藏挖空（返回 false = 不显示全部）
    // cancel = 显示答案（返回 true = 显示全部）
...
const onExport = async (type) => {
  ...
  // 对于图片/PDF 等视觉化导出，先询问用户挖空显隐偏好
  let clozeShowAll = null // null=不需要挖空处理，true=显示全部，false=隐藏全部
  const needsClozeHandling = CLOZE_VISUAL_EXPORT_TYPES.has(type)
  if (needsClozeHandling) {
    try {
      if (hasAnyCloze()) {
        const showAll = await askClozeExportPreference()
        if (showAll === null) return // 用户取消
        clozeShowAll = showAll
      }
    } catch (e) {
      console.warn('[FixedToolbar] 挖空导出设置失败，继续默认导出:', e)
    }
  }
  ...
  // 应用挖空导出状态（仅视觉化导出且有挖空内容时）
  let clozeSnapshot = null
  if (clozeShowAll !== null) {
    try {
      clozeSnapshot = setClozeStateForExport(clozeShowAll)
      // 等待一帧确保样式应用完成
      await new Promise(resolve => requestAnimationFrame(resolve))
    } catch (e) {
      console.warn('[FixedToolbar] 应用挖空导出状态失败:', e)
    }
  }
  ...
  try {
    if (type === 'pdf-wechat' || type === 'pdf-feishu') {
      ...
      // 发送成功后删除默认文件夹里的临时导出文件，避免每次「导出并发送」都残留一份
      try {
        await window.electronAPI?.fs?.remove?.(saved.filePath)
      } catch (e) {
        // 删除失败静默忽略，不影响已完成的发送
      }
      return
    ...
    if (clozeSnapshot) {
      try {
        restoreClozeStateForExport(clozeSnapshot)
      } catch (e) {
        console.warn('[FixedToolbar] 恢复挖空状态失败:', e)
      }
    }
    exporting.value = false
  }
