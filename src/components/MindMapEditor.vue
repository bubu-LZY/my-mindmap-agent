...
function normalizeNodeFields(node, uidSet) {
  ...
  while (uidSet.has(node.data.uid)) {
...
/**
 * 判断节点是否在视口内（带 50px 缓冲，避免边缘节点也触发移动）
 * 用于方向键导航时决定是否需要 moveNodeToCenter，减少视图跳动
 * 使用 DOM getBoundingClientRect 方式判断，不依赖库内部 viewport 数据结构
 */
const isNodeInViewport = (node) => {
  try {
    if (!node || !containerRef.value) return false
    // 获取节点对应的 DOM 元素
    const nodeEl = node._textData?.node?.node
    if (!nodeEl || !nodeEl.getBoundingClientRect) return false
    const nodeRect = nodeEl.getBoundingClientRect()
    const containerRect = containerRef.value.getBoundingClientRect()
    // 50px 缓冲（CSS 像素）
    const padding = 50
    return nodeRect.left >= containerRect.left + padding &&
...
const navigateActiveNode = (direction) => {
  ...
  if (targetNode) {
    ...
    try {
      ...
      // 仅当节点不在视口内时才移动到中心，避免频繁跳动
      if (typeof mindMap.renderer.moveNodeToCenter === 'function' && !isNodeInViewport(targetNode)) {
...
const crossNodeArrowNavigate = (e) => {
  ...
  // 兜底：getSelection() 返回 null 时（如焦点短暂丢失），用插件缓存的 range/lastRange
  if (!sel) {
    try {
      sel = rt.range || rt.lastRange || null
    } catch (_) { sel = null }
  }
...
const onQuillKeydownCapture = (e) => {
  try {
    ...
    // Tab / Shift+Tab：编辑状态下也支持添加子节点 / 提升节点层级
    if (e.key === 'Tab') {
      const hasActiveNode = mindMap && mindMap.renderer && mindMap.renderer.activeNodeList && mindMap.renderer.activeNodeList.length > 0
      if (hasActiveNode) {
        e.preventDefault()
...
/**
 * 选中节点时复制纯文本（不带 HTML 标签）
 * 拦截 document 级 copy 事件，当有选中节点且不在编辑状态时，
 * 将选中节点的纯文本内容写入剪贴板，避免复制出 HTML 标签。
 */
const onNodeCopyCapture = (e) => {
  try {
    if (!mindMap || !mindMap.renderer) return
    // 如果正在编辑文本，交给 Quill 自己处理（编辑态的 copy 已经处理过）
    const activeEl = document.activeElement
    if (activeEl && activeEl.classList && activeEl.classList.contains('ql-editor')) return
    // 如果目标是可编辑元素（input/textarea/contenteditable），不拦截
    const target = e.target
    if (target) {
...
const setupLinkInteraction = () => {
  ...
  // 选中节点时复制纯文本
  document.addEventListener('copy', onNodeCopyCapture, true)
...
onBeforeUnmount(() => {
  ...
  // 清理选中节点复制纯文本的事件监听
  document.removeEventListener('copy', onNodeCopyCapture, true)
  // 清理富文本内嵌图片交互监听
