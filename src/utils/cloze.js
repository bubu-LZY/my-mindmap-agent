...
/**
 * 检测整个思维导图中是否有任何挖空内容
 * 用于导出时决定是否需要显示挖空显隐选择弹窗
 */
export const hasAnyCloze = () => {
    ...
    if (node) {
      ...
      try {
        ...
        if (r && Array.isArray(r.activeNodeList) && !r.activeNodeList.includes(node)) {
          // 节点未激活时才激活；已激活（含编辑态）时跳过，
          // 避免触发 before_node_active 导致正在编辑的节点莫名退出编辑
...
/**
 * 导出前临时设置挖空显隐状态，返回可恢复的状态快照。
 * 导出结束后用 restoreClozeStateForExport 恢复，不改变用户的真实显隐设置。
 * @param {boolean} showAll - true=显示所有挖空（带答案），false=隐藏所有挖空（不带答案）
 */
export const setClozeStateForExport = (showAll) => {
  const snapshot = {
    hiddenAll,
...
/**
 * @deprecated 请使用 setClozeStateForExport(true) 替代
 */
export const forceShowAllClozeForExport = () => {
  return setClozeStateForExport(true)
}
