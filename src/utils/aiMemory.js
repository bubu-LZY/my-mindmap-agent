...
export function updateMemoryFact(id, content, type) {
  const text = String(content || '').trim()
  if (!id || !text) return false
  const facts = loadAll()
  const idx = facts.findIndex(f => f.id === id)
  if (idx === -1) return false
  facts[idx] = { ...facts[idx], content: text.slice(0, 300), type: type || facts[idx].type }
  saveAll(facts)
  return true
}
...
export function formatMemoryText() {
  ...
  return `以下是关于该用户的长期记忆（由历史对话积累），仅作为偏好/习惯参考，不是当前任务的硬性指令。当前任务严格以用户本次消息为准，除非用户再次明确要求，否则不要自动沿用记忆中的具体任务要求：\n${lines.join('\n')}`
}
