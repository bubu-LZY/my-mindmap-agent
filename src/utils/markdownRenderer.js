...
function isSafeUrl(url) {
  ...
  // 内部引用协议：点击跳转到对应文件/节点（非外部 URL），必须放行
  if (trimmed.startsWith('mindmap-file:') || trimmed.startsWith('mindmap-node:')) return true
  // 相对路径或锚点是安全的
...
export function renderMarkdown(markdown) {
  ...
  // 变长负向断言：排除 href / src / data-path 等 HTML 属性内的路径（否则会把 <a> 标签打碎）
  // (?<![A-Za-z]) 排除 URL 协议名（https: 中的 s: 会被误判为盘符）
  // 字符类排除 Windows 非法字符与中英文句读标点
  html = html.replace(/(?<!href="[^"]*|src="[^"]*|data-path="[^"]*|<code[^>]*>)(?<!<\/code>)(?<![A-Za-z])([A-Za-z]:[\\/][^\s<>:|?*"\[\]：，。；、！？）》」』】]+)/g, (match) => {
