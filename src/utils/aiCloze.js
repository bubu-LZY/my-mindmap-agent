...
const buildSmartSystemPrompt = () => {
  ...
  return `你是思维导图智能挖空助手。为每个节点挑选「忘了它就答错/答不全/会混淆」的关键区分词做挖空，用于背诵复习。

${mindMapTypePrompt(mapType, 'cloze')}

## 唯一核心原则：宁缺毋滥
只挖真正的得分点：核心术语、专业概念、关键数字、人名地名日期、重要结论、限定词（唯一/最/基本/首要/核心/前提/根本）。
不挖：普通名词、铺垫描述、举例、修饰词、功能词（的/了/是/在/和/与/或）、章节或概括性大标题。
没有明确重点 → 该节点返回空 clozes。绝不为了凑数量而挖无关紧要的词。

## 判断步骤（内部推理，不要输出）
1. 结合 parentText、siblingsTexts 找出该节点与同级节点的区分点。
2. 「标签：值」格式只挖冒号后的「值」，保留标签作提示。
3. 挖完后自检：剩余文字 + 上下文能否推出被挖内容？推不出就别挖。

## 必须遵守
1. 挖空片段必须与节点原文（或改写后文本）逐字完全匹配，含标点。
2. 根节点（level 0）不挖。同一关键词在同一节点只挖一次。
3. 一句话的核心语义成分不能全挖光，至少留一个可推断线索。
4. 并列短类别（父节点是「特点/表现/原因/类型/方法」等分类词，子节点是「客观性/普遍性」这类 ≤6 字的并列项）：整词挖空这些区分性短类别，可不留残余。
5. 若 childrenTexts 非空且 text 本身是子内容的标题/概括/名称（如「含义」「特点」「形式」「理性认识」，很短且子节点是对它的展开）→ 返回空 clozes（挖掉标题会丢失背诵框架）。但若 text 是完整陈述句（如「定义：抽象思维对事物本质的把握」），仍正常挖「值」部分。

## 挖空数量上限（是上限，不是下限；没重点就挖 0 个）
- ≤10 字：最多 1 个；10~25 字：最多 2 个；>25 字：最多 3 个。

## 改写（仅在原文确实不适合直接挖空时）
整段长句、无明确关键词、口语冗长的节点，可改写后挖空：rewrite 输出精炼书面化文本（保留全部关键知识点、不改变原意、不增删考点），clozes 针对改写后文本输出并与其完全匹配。能直接挖空的节点绝不改写。

## 返回格式（严格遵守）
只输出合法 JSON（可包在 \`\`\`json 代码块中），禁止尾随逗号、注释、NaN、解释文字：
{"results": [{"uid": "节点uid", "clozes": ["片段1"]}, {"uid": "另一个uid", "clozes": ["片段1", "片段2"]}, {"uid": "需改写的uid", "rewrite": "改写后文本", "clozes": ["改写后要挖空的片段"]}]}
uid 必须与输入完全一致；没有需要挖空的节点返回 {"results": []}。`
}
...
// 边界标点/空白：任何情况下都可以从关键词两端安全剥离。
const BOUNDARY_PUNCT = '\\s\\u3000，。！？；、,.!?;:：\'"“”‘’（）()\\[\\]【】《》<>·\\-—–~～*#`>|'
// 连接词/助词：只保留「几乎不会出现在真正关键词首尾」的高精度集合。
// 刻意不含 把/被/将/所/之/其/此/该/对/中/里/就/还/又/并 等——它们常是词的内部语素
// （和谐、被动、所有制、集中、目的、其中…），盲剥会误伤关键词本身。
const LEADING_JUNK_RE = new RegExp(`^(?:[${BOUNDARY_PUNCT}]+|的|了|着|过|地|得|和|与|及|或|而|也)+`)
const TRAILING_JUNK_RE = new RegExp(`(?:[${BOUNDARY_PUNCT}]+|的|了|着|过|地|得|和|与|及|或|而|也|等)+$`)

// 剥离挖空短语里被贪婪正则带进来的助词、连接词与标点残片：
// 「揭示了规律」→「规律」，「示了矛盾」→「矛盾」，「和客观性」→「客观性」，「普遍性等」→「普遍性」。
// 只剥离边界处的助词/连接词/标点，保留短语内部构成（如「事物的本质」中间的「的」不动）。
const sanitizePhrase = (phrase) => {
  ...
  // 2) 若仍含「了」且不在开头（「动词+了+名词」残片），从最后一个「了」后截取
  const lastLe = s.lastIndexOf('了')
  if (lastLe > 0 && lastLe < s.length - 1) {
    s = s.slice(lastLe + 1)
  }
  // 3) 去掉结尾的标点/助词/连接词（防御性处理）
  s = s.replace(TRAILING_JUNK_RE, '')
  // 4) 剥离后可能又暴露出新的开头残片（如「等」被削掉后剩下连接词），再清一次开头
  s = s.replace(LEADING_JUNK_RE, '')
  // 安全闸：若剥离后不足 2 字，说明首/尾字其实是词的语素（和谐→谐、平等→平、目的→目），
  // 此时保留原短语，宁可少剥也不误伤关键词本身。
...
const extractClozeCandidates = (text) => {
  ...
  const add = (phrase, priority) => {
    // 清洗掉贪婪正则带进来的助词/动词残片，避免把「了」等助词一起挖空
    const cleaned = sanitizePhrase(phrase)
    if (isUsablePhrase(cleaned) && text.includes(cleaned)) {
      candidates.push({ phrase: cleaned, priority })
    }
  }
  ...
  // ===== 优先级 6（最高）：冒号后面的内容（核心考点，必须挖空） =====
  // 支持「标题：值」结构，值可能是并列词（顿号/和/与/及分隔）或短语
  const colonRe = /[:：]\s*([^。；！？\n]+)/g
  let cm
  while ((cm = colonRe.exec(text)) !== null) {
    const seg = cm[1].trim()
    if (seg.length < 2) continue
    const items = seg.split(/[、和与及]/).map(s => s.trim()).filter(Boolean)
    if (items.length >= 2) {
      items.slice(0, 3).forEach(item => add(item, 60))
    } else if (seg.length <= 15) {
      add(seg, 60)
    } else {
      add(seg.slice(0, 12), 55)
    }
  }
...
const buildEnhancedFallbackClozeList = (nodes) => {
  ...
  nodes.forEach(node => {
    ...
    if (clozes.length === 0) {
      let simple = pickFallbackCloze(text)
      if (simple) {
        simple = sanitizePhrase(simple)
        if (isUsablePhrase(simple) && text.includes(simple)) {
          clozes = [simple]
        }
      }
    }
...
const removeClozeForKeyword = (node, keyword) => {
  ...
  div.querySelectorAll('.smm-cloze').forEach(span => {
    if (span.textContent === keyword) {
      ...
      if (!parent || parent !== span.parentNode) return // 防御：父节点不存在或已变更
      while (span.firstChild) parent.insertBefore(span.firstChild, span)
      if (parent.contains(span)) { // 防御：确保 span 仍是 parent 的子节点
...
const doSmartCloze = async (nodes, mode, onProgress, options = {}) => {
  // aiEnabled === false 表示「仅兜底挖空」：只走正则/结构规则，不调用 AI
  ...
  // ========== 步骤1：机器（正则/结构模式）先挖，无条件执行 ==========
  if (onProgress) {
    onProgress(aiEnabled ? '正在快速分析节点并生成初步挖空…' : '正在按规则进行挖空…')
  }
  try {
    ...
    flushRender()
    if (onProgress) {
      onProgress({ phase: 'fallback_done', message: aiEnabled ? '快速挖空完成，AI补挖遗漏关键词…' : '规则挖空完成' })
    }
  } catch (e) {
    console.error('[AI挖空] 机器挖空失败，继续AI处理:', e)
    clozeList = []
  }

  // ========== 仅兜底挖空：只跑正则/结构规则，不调用 AI，直接返回 ==========
  if (!aiEnabled) {
    flushRender()
    if (totalApplied === 0) {
      throw new Error('未能从当前内容中识别出可挖空的关键词，请检查节点内容后重试')
    }
    return {
      count: totalApplied,
      aiCount: aiApplied,
  ...
  // ========== 步骤2：AI 挖剩下的（机器没挖到关键词的节点） ==========
  const machineCovered = new Set(clozeList.filter(c => c && c.uid).map(c => c.uid))
  const unclozedNodes = nodes.filter(n => n && n.uid && !machineCovered.has(n.uid))
  if (unclozedNodes.length > 0 && !aiService.isAborted()) {
    try {
      const aiResult = await callAiForClozeBatched(unclozedNodes, mode, onProgress, applyBatch)
      if (Array.isArray(aiResult)) {
        clozeList = clozeList.concat(aiResult.filter(r => r && r.uid))
      }
    } catch (e) {
      flushRender()
      if (!(e && e.aborted)) {
        console.warn('[AI挖空] AI补挖失败，保留机器挖空结果:', e)
      }
    }
  }
 ...
 */
export const smartClozeNodes = async (nodeList, mode = 'smart', onProgress, options = {}) => {
  const mindMapRef = getMindMapRef()
 ...
 */
export const smartClozeFullMap = async (mode = 'smart', onProgress, options = {}) => {
  const mindMapRef = getMindMapRef()
  if (!mindMapRef || !mindMapRef.renderer) {
    throw new Error('思维导图未初始化')
  }
