  ...
  <div class="app-container">
    ...
    <header class="top-navbar">
      ...
      <div class="navbar-actions">
        <button class="nav-btn feishu-btn" title="消息中心（微信端、飞书端、定时任务的消息记录）" @click="messageCenterVisible = !messageCenterVisible">
          ...
          <span class="feishu-btn-text">消息中心</span>
        </button>
    ...
    <div class="main-content">
      ...
      <aside class="ai-panel" :class="{ collapsed: !aiPanelExpanded, 'floating-chat': !aiPanelExpanded && floatingChatVisible, 'in-fullscreen': mmFullscreen || outlineFullscreen }">
        ...
        <ChatPanel
          ...
          @open-reference="handleOpenReferenceFile"
    ...
    <!-- ============ 消息中心（悬浮面板，第三方消息记录） ============ -->
    <FloatingMessageCenter v-if="messageCenterVisible" @close="messageCenterVisible = false" />
...
// 拖动标签栏「标签组」标签时的源组 id（用于标签组排序 / 拖回并入）
...
// 标签组（浏览器标签组化）：每项 { id, root: <pane|split>, activePaneId }
// 单 pane 组 = 普通文件标签；多 pane 组 = “多屏”组合标签。多组并存即可“拖出/拖回”。
...
// 激活组（activeGroupId 失效时兜底到第一个组，保证 layoutRoot 永远指向有效树）
const activeGroup = computed(() =>
  layoutGroups.value.find(g => g.id === activeGroupId.value) || layoutGroups.value[0] || null
)

// 兼容层：旧代码直接读写 layoutRoot（单棵分屏树）。
// 读 = 激活组的树；写 = 写入激活组（无组时自动建组）。单组时行为与旧版完全一致，
// 全部旧引用（布局计算/分屏/持久化/模板）无需逐一迁移。
const layoutRoot = computed({
  get: () => (activeGroup.value ? activeGroup.value.root : null),
  set: (v) => {
    const g = activeGroup.value
    if (!g) {
      const ng = { id: nextGroupId(), root: v, activePaneId: '' }
      layoutGroups.value.push(ng)
      activeGroupId.value = ng.id
      return
    }
    g.root = v
  }
...
// 兼容层：当前激活 pane id，实际存储在激活组上，切换组时自动跟随
const activePaneId = computed({
  get: () => (activeGroup.value ? activeGroup.value.activePaneId : ''),
  set: (v) => { if (activeGroup.value) activeGroup.value.activePaneId = v }
})
...
// 跨组按 paneId 查找：返回 { group, pane } 或 null
const findPaneByIdAnyGroup = (paneId) => {
  for (const g of layoutGroups.value) {
    const p = findPaneById(g.root, paneId)
    if (p) return { group: g, pane: p }
  }
  return null
}

// 跨组按文件查找 pane。不变量：同一文件同一时刻全局只有一个 pane
// （拖出是“移动”pane，openFileToPane/splitPane 已存在的文件聚焦原 pane），因此查找结果唯一。
const findPaneByFileAnyGroup = (fileType, key) => {
  for (const g of layoutGroups.value) {
    const p = findPaneByFile(g.root, fileType, key)
    if (p) return p
  }
  return null
}

// 收集所有组的 pane（文件重命名/移动后同步 pane 信息用）
const collectAllPanes = () => {
  const out = []
  layoutGroups.value.forEach(g => collectPanes(g.root, out))
  return out
}
...
// 是否多分屏模式（激活组内大于 1 个 pane 时显示边框/标题栏）
const isMultiPane = computed(() => collectPanes(layoutRoot.value).length > 1)

// ===== 标签组（标签栏渲染）辅助 =====
const groupPanes = (g) => collectPanes(g?.root)
const isEmptyGroup = (g) => !g || !g.root
const isGroupMulti = (g) => groupPanes(g).length > 1
const groupTabName = (g) => isEmptyGroup(g) ? '新标签页' : (groupPanes(g)[0]?.title || '未命名')
const groupTabTitle = (g) => {
  if (isEmptyGroup(g)) return '新标签页'
  const ps = groupPanes(g)
  if (ps.length > 1) return ps.map(p => p.title || '未命名').join(' / ')
  return ps[0]?.title || '未命名'
}
// 多屏组合标签：合并显示所有 pane 的名称（前三字 + 省略号，蓝色竖杠分隔）
...
// 聚焦 pane：跨组定位（聚焦其它组的 pane 时自动切换激活组），同步单值状态（activeFileId / activeDocFilePath / viewMode / showDocViewer）
const focusPane = (paneId) => {
  const hit = findPaneByIdAnyGroup(paneId)
  if (!hit) return
  const { group, pane } = hit
  if (activeGroupId.value !== group.id) activeGroupId.value = group.id
...
// 打开文件到分屏：已存在（可能在其它标签组）则切组聚焦；空布局则单 pane；否则替换激活 pane（一个界面一个导图）
const openFileToPane = (fileType, fileId, filePath, title) => {
  const key = fileType === 'doc' ? filePath : fileId
  if (!key) return null
  let pane = findPaneByFileAnyGroup(fileType, key)
  if (pane) {
    focusPane(pane.id)
    return pane
  }
  ...
  // 打开新文件 = 新建独立标签组（浏览器标签语义）：不再替换激活窗口，
  // 否则被替换的文件会失去 pane 却仍残留在 tabs 中，导致标签与窗口不一致。
  pane = makePane(fileType, { fileId, filePath, title })
  const ng = { id: nextGroupId(), root: pane, activePaneId: pane.id }
  layoutGroups.value.push(ng)
  activeGroupId.value = ng.id
  focusPane(pane.id)
  return pane
...
// 把 pane 节点并入树：在 targetPaneId 的 direction（left/right/top/bottom）侧插入。
// splitPane 分屏与“标签组拖回并入”共用此逻辑。
const insertPaneIntoTree = (root, targetPaneId, direction, newPane) => {
  const replace = (node) => {
    if (!node) return null
    if (node.kind === 'pane') {
...
const splitPane = (targetPaneId, direction, fileType, fileId, filePath, title) => {
  const key = fileType === 'doc' ? filePath : fileId
  if (!key) return null
  // 同一文件全局只保留一个 pane：若已存在（可能在其它组），切组聚焦已有窗口，避免重复分屏导致串文件。
  const existing = findPaneByFileAnyGroup(fileType, key)
  if (existing) {
  ...
  layoutRoot.value = insertPaneIntoTree(layoutRoot.value, targetPaneId, direction, newPane)
...
// 关闭 pane：从所属组移除并折叠；组空则删除该组并聚焦相邻组；
// 文件在所有组中无其它 pane 引用则联动关闭标签。
// 返回 true 表示已关闭；false 表示被用户取消（保存失败时选择不关闭）。
const closePane = async (paneId) => {
  ...
  group.root = remove(group.root)

  if (pane.fileType === 'mindmap' && !pane.empty) {
    if (!findPaneByFileAnyGroup('mindmap', pane.fileId)) {
      const idx = tabs.value.findIndex(t => t.fileId === pane.fileId)
      if (idx >= 0) tabs.value.splice(idx, 1)
      releaseAiBindIfClosed(pane.fileId)
    }
  } else if (pane.fileType === 'doc') {
    if (!findPaneByFileAnyGroup('doc', pane.filePath)) {
      docTabs.value = docTabs.value.filter(t => t.filePath !== pane.filePath)
    }
  }

  if (!group.root || collectPanes(group.root).length === 0) {
    // 组已空：删除组（聚焦相邻组 / 回到欢迎页）
    removeGroup(group.id)
  } else {
    // 组内还有 pane：若激活 pane 正是被关的，聚焦组内第一个 pane
    if (!findPaneById(group.root, group.activePaneId)) {
      group.activePaneId = collectPanes(group.root)[0]?.id || ''
    }
    if (group.id === activeGroupId.value && activePaneId.value === paneId && group.activePaneId) {
      focusPane(group.activePaneId)
    }
  }
...
// 删除标签组：若删的是激活组则聚焦相邻组，无相邻组则回到欢迎页
const removeGroup = (groupId) => {
  const gi = layoutGroups.value.findIndex(g => g.id === groupId)
  if (gi < 0) return
  const group = layoutGroups.value[gi]
  const wasActive = group.id === activeGroupId.value
  layoutGroups.value.splice(gi, 1)
  if (wasActive || !activeGroup.value) {
    const nextG = layoutGroups.value[Math.min(gi, layoutGroups.value.length - 1)]
    const first = nextG && (findPaneById(nextG.root, nextG.activePaneId) || collectPanes(nextG.root)[0])
    if (first) {
      focusPane(first.id)
    } else {
      resetToWelcomeState()
...
// 统一 resize 所有已注册编辑器（分屏结构变化后重排画布/文档，避免空白或缩放残留）
...
// 激活空白组：切换到该组并清空单值状态（画布显示欢迎页）
const activateEmptyGroup = (groupId) => {
...
// 聚焦标签组：激活该组并聚焦其激活 pane（无激活 pane 则聚焦组内第一个）
const focusGroup = (groupId) => {
  const g = layoutGroups.value.find(x => x.id === groupId)
  if (!g) return
  if (isEmptyGroup(g)) {
    activateEmptyGroup(groupId)
    return
  }
  const p = findPaneById(g.root, g.activePaneId) || collectPanes(g.root)[0]
  if (p) focusPane(p.id)
}

// 拖出：把多屏组合标签里的某一个 pane 摘出来，成为独立标签组（插在原组之后）
const detachPaneToOwnGroup = (paneId) => {
  const hit = findPaneByIdAnyGroup(paneId)
  if (!hit) return
  const { group, pane } = hit
  if (collectPanes(group.root).length <= 1) return // 单 pane 组无“拖出”可言
  // 从组树摘除该 pane，并折叠空 split（只剩一个兄弟时上移）
  const remove = (node) => {
    if (!node) return null
    if (node.kind === 'pane') return node.id === paneId ? null : node
    const children = node.children.map(remove).filter(Boolean)
  ...
  group.root = remove(group.root)
  // 若摘掉的是原组激活 pane（或激活 pane 已失效），改指组内第一个
  if (!findPaneById(group.root, group.activePaneId)) {
    group.activePaneId = collectPanes(group.root)[0]?.id || ''
  }
  // 新建独立组，插在原组之后，并聚焦
  const ng = { id: nextGroupId(), root: pane, activePaneId: paneId }
  const idx = layoutGroups.value.indexOf(group)
  layoutGroups.value.splice(idx + 1, 0, ng)
  activeGroupId.value = ng.id
  focusPane(paneId)
  resizeAllEditors()
}

// 拖回/合并：把 sourceGroup 整组并入 targetGroup，形成左右分屏组合；源组从列表移除。
// 单 pane 拖到多屏组 = 拖回并入；两个单 pane 拖到一起 = 合并成多屏组合。
const mergeGroupInto = (targetGroupId, sourceGroupId) => {
  if (targetGroupId === sourceGroupId) return
  const srcIdx = layoutGroups.value.findIndex(g => g.id === sourceGroupId)
  const tgt = layoutGroups.value.find(g => g.id === targetGroupId)
  if (srcIdx < 0 || !tgt) return
  const src = layoutGroups.value[srcIdx]
  const srcRoot = src.root
  if (!srcRoot) return
  if (!tgt.root) {
    // 目标为空组：直接把源组树移入目标组（不额外包 split）
    tgt.root = srcRoot
  } else {
    // 合并成左右分屏（保持原有两棵树不变，pane 对象原样搬移，全局唯一不变量不受影响）
    tgt.root = {
      id: nextSplitId(),
      kind: 'split',
      direction: 'row',
      sizes: [0.5, 0.5],
      children: [tgt.root, srcRoot]
    }
  }
  const firstPaneId = (src.activePaneId && findPaneById(srcRoot, src.activePaneId)?.id)
    || collectPanes(srcRoot)[0]?.id
    || ''
  layoutGroups.value.splice(srcIdx, 1)
  activeGroupId.value = targetGroupId
  if (firstPaneId) {
    tgt.activePaneId = firstPaneId
    focusPane(firstPaneId)
  }
  resizeAllEditors()
}

// 拖组标签（独立标签）到多屏组合的某个 pane 边缘 → 并入该组并在该方向分屏
const mergeGroupIntoPane = (targetPaneId, edge, sourceGroupId) => {
  const targetHit = findPaneByIdAnyGroup(targetPaneId)
  if (!targetHit) return
  const targetGroup = targetHit.group
  if (targetGroup.id === sourceGroupId) return
  const srcIdx = layoutGroups.value.findIndex(g => g.id === sourceGroupId)
  if (srcIdx < 0) return
  const src = layoutGroups.value[srcIdx]
  const srcRoot = src.root
  ...
  // 源组整棵树作为子树，插入目标组在 targetPaneId 的 edge 方向
  targetGroup.root = insertPaneIntoTree(targetGroup.root, targetPaneId, edge, srcRoot)
  layoutGroups.value.splice(srcIdx, 1)

  // 聚焦并入的源组第一个 pane
  const srcPaneId = (src.activePaneId && findPaneById(srcRoot, src.activePaneId)?.id)
    || collectPanes(srcRoot)[0]?.id
    || ''
  activeGroupId.value = targetGroup.id
  if (srcPaneId) {
    targetGroup.activePaneId = srcPaneId
    focusPane(srcPaneId)
  }
  resizeAllEditors()
}

// 关闭整个标签组：依次关闭组内所有 pane（任一 pane 保存取消则中断）
const closeGroup = async (groupId) => {
  const g = layoutGroups.value.find(x => x.id === groupId)
  if (!g) return
  // 空组（未打开任何文件）：直接删除
  if (!g.root || collectPanes(g.root).length === 0) {
    removeGroup(groupId)
    return
  }
  const paneIds = collectPanes(g.root).map(p => p.id)
...
// 点击标签栏「+」新建空白标签组（root = null 显示欢迎页，可在此打开文件或继续分屏）
const createEmptyGroup = () => {
  const ng = { id: nextGroupId(), root: null, activePaneId: '' }
  layoutGroups.value.push(ng)
  activateEmptyGroup(ng.id)
  return ng
}

// 所有标签组关闭后的空白态（对应旧版“最后一个 pane 关闭”分支）
...
// 持久化监听整个标签组数组：非激活组的变更（拖出/并入/重排）也要存盘
watch(layoutGroups, schedulePersistLayout, { deep: true })
...
const onPaneDrop = async (pane, e) => {
  ...
  // 拖组标签（独立标签）到 pane 边缘 → 并入目标 pane 所在组（在指定方向分屏）
  if (dragGroupId.value) {
    const sourceGroupId = dragGroupId.value
    const edge = computeDropEdge(pane, e) || 'right'
    dragGroupId.value = ''
    dragOverPaneId.value = ''
    dragOverEdge.value = ''
    mergeGroupIntoPane(pane.id, edge, sourceGroupId)
    return
  }
  dragOverPaneId.value = ''
  dragOverEdge.value = ''
  ...
  // 排除标签组/pane 标题栏的拖拽（它们只在标签栏/pane 间生效，落到 pane 上不算分屏）
  if (!text || text.startsWith('pane:') || text.startsWith('group:')) return
  const edge = computeDropEdge(pane, e) || 'right'
...
const persistLastLayout = () => {
  try {
    // 空组（未打开文件）不持久化，避免恢复时被误报为“文件不存在”
    const groups = layoutGroups.value.filter(g => g && g.root)
    if (!groups.length) {
      localStorage.removeItem(LAST_LAYOUT_KEY)
      return
    }
    // v2：多标签组布局（旧版 v1 存单棵树，restoreLastLayout 负责迁移）
    const activeIdx = Math.max(0, groups.findIndex(g => g.id === activeGroupId.value))
    const data = {
      version: 2,
      activeGroupIndex: activeIdx,
      groups: groups.map(g => ({ root: g.root, activePaneId: g.activePaneId }))
    }
    localStorage.setItem(LAST_LAYOUT_KEY, JSON.stringify(data))
  } catch {
    // 布局持久化失败不影响使用
  }
}
...
const restoreLastLayout = async () => {
  ...
  let saved = null
  try {
    saved = JSON.parse(localStorage.getItem(LAST_LAYOUT_KEY) || 'null')
  } catch {
    saved = null
  }
  if (!saved) return
  try {
    // v1 单棵树 → 迁移为单组；v2 → 逐组重建
    const groupDefs = []
    if (Array.isArray(saved.groups)) {
      for (const g of saved.groups) {
        if (g && g.root) groupDefs.push({ root: g.root, activePaneId: g.activePaneId || '' })
      }
    } else if (saved.kind) {
      groupDefs.push({ root: saved, activePaneId: '' })
    }
    if (!groupDefs.length) return
    const restored = []
    let skipped = 0
    for (const def of groupDefs) {
      const root = await buildPaneTree(def.root)
      if (!root) { skipped++; continue }
      const g = { id: nextGroupId(), root, activePaneId: '' }
      const target = findPaneById(root, def.activePaneId) || collectPanes(root)[0]
      if (target) g.activePaneId = target.id
      restored.push(g)
    }
    if (!restored.length) return
    layoutGroups.value = restored
    const activeIdx = Number.isInteger(saved.activeGroupIndex) ? saved.activeGroupIndex : 0
    const first = restored[Math.min(Math.max(activeIdx, 0), restored.length - 1)]
    const focus = findPaneById(first.root, first.activePaneId) || collectPanes(first.root)[0]
    if (focus) focusPane(focus.id)
    if (skipped > 0) {
      ElMessage.info(`有 ${skipped} 个标签布局中的文件已不存在，已跳过`)
    }
  } catch {
    // 上次布局失效时静默回退到空白界面
  }
...
const restoreLayoutTemplate = async (tpl) => {
  ...
  try {
    ...
    // 模板恢复 = 用该布局重置整个标签栏（与旧版整树替换语义一致），其余标签组会被丢弃
    layoutGroups.value = [{ id: nextGroupId(), root, activePaneId: '' }]
...
// AI 实际操作的导图实例：锁定到某窗口时用该窗口实例（跨标签组定位），否则跟随当前聚焦窗口
const aiBoundMindMap = computed(() => {
  if (aiBindLocked.value && aiBindFileId.value) {
    const pane = findPaneByFileAnyGroup('mindmap', aiBindFileId.value)
    if (pane) {
...
const handleMarkdownApply = ({ tree, fileId }) => {
  if (!tree) return
  const fid = fileId || activeFileId.value
  const pane = findPaneByFileAnyGroup('mindmap', fid) || activePane.value
...
// ChatPanel 返回按钮：导航到指定文件
const onNavigateToFile = (filePath, fileName) => {
  if (!filePath) return
  hasFile.value = true
  nextTick(() => {
    if (fileTreeRef.value && fileTreeRef.value.openFileByPath) {
...
/**
 * 打开引用文件（悬浮窗"去编辑" / "去查看"）
 * - 思维导图（含可解析到同名 .smm 的 Markdown）→ 加载并编辑
 * - PDF / Word / Excel / PPT / CSV / 纯文本 / Markdown 等文档 → 文档查看器只读查看
 */
const handleOpenReferenceFile = async ({ filePath, nodeUid }) => {
  if (!filePath) return
  try {
    // 解析最终目标与动作：导图→编辑，PDF/Word/Excel/普通 Markdown 等→只读查看
    const act = await resolveRefAction(filePath)
    // 切换前先落盘当前文件的待保存修改
    await flushAutoSave()

    if (act.action === 'view') {
      onTreeOpenDoc({ filePath, fileName: getBaseName(filePath) })
      if (act.source === 'md') {
        ElMessage.info('Markdown 已用文档查看器打开（只读），可右键「AI 转换为思维导图」')
      }
      return
    }

    // Markdown 内容即导图大纲：落盘为同名 .smm（存到默认保存位置，源文件不动）后打开编辑
    let openPath = act.filePath
    if (act.source === 'md-outline' && act.tree) {
      const base = getBaseName(filePath).replace(/\.(md|markdown)$/i, '') || '未命名'
      const saved = await window.electronAPI?.saveFile?.(`${base}.smm`, act.tree)
      if (saved && saved.success && saved.filePath) {
        openPath = saved.filePath
        ElMessage.success(`已导入为 ${getBaseName(saved.filePath)}（原 Markdown 保持不变）`)
      } else {
        // 落盘失败不静默：退回只读查看，避免点了没反应
        ElMessage.warning(`生成导图失败：${saved?.error || '未知错误'}，已按只读打开`)
        onTreeOpenDoc({ filePath, fileName: getBaseName(filePath) })
        return
      }
    }
    ...
    if (currentFilePath.value && currentFilePath.value !== openPath) {
    ...
    if (act.source === 'md') {
      ElMessage.success(`已打开同名思维导图：${getBaseName(openPath)}`)
    }

    const result = await window.electronAPI.refReadFile(openPath)
    if (!result.success) {
    ...
    if (result.type === 'json' && data) {
      ...
      currentFilePath.value = openPath
      currentFileIsImported.value = false
      markClean(data)
      // [多实例] 同步 activeFileId 到目标文件并确保 Tab 存在
      syncActiveTab(openPath, data, false)
      if (editorRef.value) {
      ...
      // 非导图结构（普通 JSON 文本 / 意外格式）：只读查看，
      // 绝不按导图加载——否则自动保存会把导图 JSON 写回源文档
      onTreeOpenDoc({ filePath: openPath, fileName: getBaseName(openPath) })
    }
  } catch (e) {
...
const loadMapFromFile = async (filePath) => {
  try {
    ...
    if (result.type === 'json' && data) {
      ...
      // 非导图结构：只读查看兜底（返回原导图的来源理应是导图，属异常防御）
      onTreeOpenDoc({ filePath, fileName: getBaseName(filePath) })
    }
  } catch (e) {
...
// [标签组] 拖拽源：拖组标签时记录源组 id，dataTransfer 带 group: 前缀以便与文件/pane 拖拽区分
const onGroupTabDragStart = (e, groupId) => {
  dragGroupId.value = groupId
  e.dataTransfer.setData('text/plain', 'group:' + groupId)
  e.dataTransfer.effectAllowed = 'move'
}
const onGroupTabDragOver = (e, groupId) => {
  // 拖 pane 标题栏（拖出）或拖组标签（排序/合并）时高亮目标标签
  if (dragPaneId.value || dragGroupId.value) {
    e.dataTransfer.dropEffect = 'move'
    dragOverTabKey.value = groupId
  }
}
const onGroupTabDragLeave = (groupId) => {
  if (dragOverTabKey.value === groupId) dragOverTabKey.value = ''
}
const onGroupTabDrop = (e, targetGroupId) => {
  dragOverTabKey.value = ''
  // 情况1：拖 pane 标题栏到标签 → 从多屏组合中“拖出”为独立标签
  if (dragPaneId.value) {
    const paneId = dragPaneId.value
    dragPaneId.value = ''
    detachPaneToOwnGroup(paneId)
    return
  }
  // 情况2：拖组标签到另一个组标签 → 拖回并入 / 合并成多屏组合
  const fromGroupId = dragGroupId.value
  dragGroupId.value = ''
  if (fromGroupId && fromGroupId !== targetGroupId) {
    mergeGroupInto(targetGroupId, fromGroupId)
    return
  }
  // 情况3：拖文件树文件到标签项 → 打开为独立标签（与拖到标签栏空白处一致）
  const text = (e.dataTransfer?.getData('text/plain') || dragTabKey.value || getDragFilePath() || '').trim()
  if (!text || text.startsWith('pane:') || text.startsWith('group:')) return
  const info = parseDropText(text)
  dragTabKey.value = ''
...
const onGroupTabDragEnd = () => {
  dragGroupId.value = ''
  dragOverTabKey.value = ''
}

// 标签栏空白处 dragover：声明可放置，确保拖出/拖回 drop 能触发
const onTabBarDragOver = (e) => {
  e.dataTransfer.dropEffect = 'move'
}
// 标签栏空白处 drop：拖 pane 标题栏 = 拖出到末尾；拖组标签 = 移到末尾；拖文件 = 打开为新标签
const onTabBarDrop = (e) => {
  if (dragPaneId.value) {
    const paneId = dragPaneId.value
    dragPaneId.value = ''
    detachPaneToOwnGroup(paneId)
  ...
  if (dragGroupId.value) {
    ...
    dragGroupId.value = ''
    const idx = layoutGroups.value.findIndex(g => g.id === gid)
    if (idx >= 0) {
      const [moved] = layoutGroups.value.splice(idx, 1)
      layoutGroups.value.push(moved)
    }
    return
  }
  const text = (e.dataTransfer?.getData('text/plain') || dragTabKey.value || getDragFilePath() || '').trim()
...
// 标签栏滚轮横向滑动：鼠标滚轮上下滚 = 标签栏左右滑动（往上滚往前，往下滚往后）
const onTabBarWheel = (e) => {
  const el = e.currentTarget
  if (!el) return
  // 触控板横向手势（deltaX 主导）交给默认横向滚动；鼠标滚轮（deltaY 主导）转为水平滑动
  if (Math.abs(e.deltaX) > Math.abs(e.deltaY)) return
  e.preventDefault()
  el.scrollLeft += e.deltaY
}

// [多实例] 关闭 Tab：联动关闭对应导图 pane（跨标签组定位；由 closePane 统一处理标签移除与折叠）
const closeTab = (fileId) => {
  const pane = findPaneByFileAnyGroup('mindmap', fileId)
  if (pane) {
    closePane(pane.id)
    return
  }
...
// 关闭文档标签：联动关闭对应文档 pane（跨标签组定位）
const closeDocTab = (filePath) => {
  const pane = findPaneByFileAnyGroup('doc', filePath)
  if (pane) {
    closePane(pane.id)
    return
  }
...
const onTreeOpenFile = async ({ filePath, fileName, data, isMarkdown, isXmind, skipAutoSave }) => {
  ...
  if (searchService.isAvailable()) {
    ...
    // 构建文件关系图谱（引用关系 + 主题相似度）
    try {
      indexFileRelations(filePath, fName, fileData)
    } catch (e) { console.warn('[关系图谱] 索引失败:', e) }
  }
}
...
const onFileRenamed = ({ oldPath, newPath }) => {
  ...
  // [分屏] 同步更新所有标签组内 pane 的 fileId / filePath
  collectAllPanes().forEach(w => {
    if (w.fileType === 'doc') {
    ...
    if (window.electronAPI && window.electronAPI.saveFile) {
      ...
      if (result && result.success) {
        ...
        if (searchService.isAvailable()) {
          ...
          try {
            indexFileRelations(result.filePath, fName, data)
          } catch (e) { console.warn('[关系图谱] 索引失败:', e) }
        }
        return true
      } else {
...
const remapImportedFileId = (oldFid, newFid) => {
  ...
  // 更新 pane（跨标签组；fileId 变化会触发编辑器按 key 重建，数据由 tab.data 兜底保持）
  collectAllPanes().forEach(w => {
    if (w.fileId === oldFid) {
      w.fileId = newFid
...
// 保存单个文件：成功/无需保存返回 true，失败返回 false（跨标签组定位编辑器实例）
const saveFileById = async (fid) => {
  const pane = findPaneByFileAnyGroup('mindmap', fid)
  ...
  if (result && result.success) {
    ...
    if (searchService.isAvailable()) {
      ...
      try {
        indexFileRelations(savedPath, fName, data)
      } catch (e) { console.warn('[关系图谱] 索引失败:', e) }
    }
    return true
  }
