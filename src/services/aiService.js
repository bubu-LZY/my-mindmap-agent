...
规则：
1. 用户需求涉及实时信息（天气/新闻/价格/汇率/政策/赛事等）→ 优先选 search_web 等联网工具。
2. 用户明确要求执行"操作"（修改/查询/导出/发送/上传/删除/切换/显示/隐藏/生成/搜索等），且目录里有对应工具时 → 选择对应工具。
3. 纯闲聊、寒暄、单纯问"能不能"但没有实际需求时，不选工具。
4. 最多选 3 个工具。

示例：
1. 用户："你好，今天天气不错"（闲聊） → {"tools":[]}
2. 用户："成都今天天气如何"（实时信息查询） → {"tools":["search_web","get_location"]}
3. 用户："把根节点改成项目总结" → {"tools":["update_node_text"]}
4. 用户："怎么导出Markdown？"（只是询问方法） → {"tools":[]}
5. 用户："帮我搜一下最新的AI新闻" → {"tools":["search_web"]}`
      ...
      while (true) {
        ...
        if (!shouldRunTools) {
          ...
          if (!anyToolCalled && !autoDiscoveryTried && latestUserQuery && !this._abortedTokens.has(token) && !isStale()) {
            ...
            if (highConfHits.length < 2 && (isRefusal || isShortUnhelpful)) {
              const semanticHits = await semanticToolMatch(latestUserQuery, toolPool, activeNames)
              // 合并结果，去重
              const seen = new Set(highConfHits.map(h => h.name))
