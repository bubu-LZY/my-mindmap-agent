...
import { applyTextStyleToNodes, applyTextStyleToTextRanges, analyzeNodeTextStyles, copyRichTextStyles, styleTextRangesByColor, colorNameToFamily, applyTextStyleToTextRangesByColor, normalizeTextColor, normalizeHighlightColor, normalizeNodeFillColor } from '../utils/textStyle'
...
// [第一仗] 会"改写"当前思维导图节点/结构/样式/挖空/复习计划的工具集合。
// 当任务绑定的 fileId 与当前打开文件不一致（用户切走了文件）时，这些工具一律拒绝执行，防止跨文件误改。
// 只读类（get_mindmap_content/search_nodes/query_node_styles/list_cloze_nodes 等）不在此列，不影响 AI 诊断。
const WRITE_GRAPH_TOOLS = new Set([
  'generate_mindmap', 'save_mindmap', 'new_mindmap',
  'expand_node', 'add_child_nodes', 'update_node_text', 'delete_node',
  'select_node', 'insert_parent_node', 'set_node_style',
  'batch_text_style', 'batch_node_actions', 'summarize_node',
  'find_replace_text', 'undo', 'redo', 'reorganize_mindmap',
  'change_layout', 'set_theme',
  'toggle_cloze_visibility', 'clear_cloze', 'set_nodes_cloze_hidden',
  'toggle_all_cloze', 'smart_cloze_nodes', 'smart_cloze_full_map',
  'apply_cloze_styles', 'mark_cycle_completed', 'mark_cycle_uncompleted',
  'clear_review_plan', 'remove_by_node_uid'
])
  ...
  switch (name) {
    ...
    case 'research_to_mindmap': {
      try {
        ...
        let fileName = null
        try {
          fileName = `${topic.replace(/[<>:"/\\|?*]/g, '_').slice(0, 50)} - research.smm`
          if (window.electronAPI?.saveFile) {
            const saveResult = await window.electronAPI.saveFile(fileName, JSON.stringify(treeData, null, 2))
            if (saveResult?.success) filePath = saveResult.filePath
          }
        } catch (error) { console.warn('研究导图自动保存失败:', error) }
