...
const toolCatalog = [
  ...
  { name: 'search_across_files', category: 'KB', desc: 'Enhanced cross-file search: results grouped by file, with related-file recommendations for expansion. Use when you need to find content across multiple files or want to discover related files after initial search.' },
  { name: 'get_related_files', category: 'KB', desc: 'Get files related to a given file (referenced files, similar-topic files, manually linked files). Use to expand your search scope or find contextually related material.' },
  ...
  { name: 'create_mcp_server', category: 'MCP', desc: 'Create/configure a new MCP server (name + transport=http(url) or stdio(command+args))' },
  ...
  { name: 'write_custom_tool', category: 'Custom', desc: 'Create/update a custom tool by writing tool.json + tool.js into the custom-tools directory (isolated subprocess, path-whitelisted file API, optional PowerShell); use when no built-in tool fits and a NEW reusable capability is needed' },
  { name: 'ask', category: 'Interaction', desc: 'Ask the user a question with choices and wait for their answer. Use this when you need clarification, confirmation, or a decision from the user before proceeding.' },
...
export const CORE_TOOL_NAMES = [
  ...
  'find_local_file',
  'save_mindmap',
  // 知识库检索（本地 SQLite + BM25 + 向量，覆盖已索引的导图与文档）
  'search_knowledge_base',
  'semantic_search',
  // 文档转导图 / 导图生成（后台离屏，全量保留原文）
  'convert_doc_to_mindmap',
  'generate_mindmap',
  // 自主能力：代码执行 + 自定义工具发现/调用/创建
  'run_code',
  'list_custom_tools',
  'call_custom_tool',
  'write_custom_tool',
...
export const aiTools = [
  ...
  },
  {
    ...
    function: {
      name: 'generate_mindmap',
      description: 'Generate a complete mindmap from Markdown content and auto-save as .smm. Put the whole Markdown into the "markdown" parameter (# = root, ## = level-1 branches, ### = level-2). It is saved to the default save dir. IMPORTANT: only for pasted/new content you compose yourself — when the user asks to turn an existing DOCUMENT/PDF/PPT into a mindmap, call convert_doc_to_mindmap(file_path=...) instead of reading it and generating a shallow outline here.',
      parameters: {
        type: 'object',
        properties: {
          markdown: { type: 'string', description: 'The COMPLETE Markdown content of the mindmap (put the text HERE, not in "content"); use # ## ### for levels' }
        },
        required: ['markdown']
      }
    }
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Enhanced cross-file search over the local knowledge base. Results are grouped by file, with related-file recommendations for second-round expansion. Use this when you need to: (1) find content across multiple files, (2) see which files are most relevant, (3) discover related files to broaden your search.',
      parameters: {
        type: 'object',
        properties: {
          query: { type: 'string', description: 'Search keyword or question' },
          maxResultsPerFile: { type: 'number', description: 'Max matching snippets per file (default 3)' },
          maxFiles: { type: 'number', description: 'Max files to return (default 5)' },
          includeRelated: { type: 'boolean', description: 'If true, also return recommended related files for expansion (default true)' }
        },
        required: ['query']
      }
    }
  },
  {
    ...
    function: {
      ...
      description: 'Get files related to a given file. Includes: files referenced by the target file, files that reference the target, files with similar topics/themes, and manually linked files. Use this to expand your search scope after finding a relevant file, or to gather all contextually related material.',
      parameters: {
        type: 'object',
        properties: {
          filePath: { type: 'string', description: 'The target file path (from search results)' },
          relationTypes: {
            type: 'array',
            items: { type: 'string' },
            description: 'Optional: filter by relation types: reference (outgoing links), referencedBy (incoming links), similar (topic similarity), manual (user-tagged). Default: all types.'
          },
          limit: { type: 'number', description: 'Max related files to return (default 5)' }
        },
        required: ['filePath']
      }
    }
  ...
  },
  {
    ...
    function: {
      name: 'reorder_nodes',
      description: 'Move a node to a specific position among its SIBLINGS (same parent). Pass uid + one of: index (0-based target index) / before_uid (place before this sibling) / after_uid (place after this sibling). Use to insert/reorder a node in the MIDDLE of siblings (e.g. insert a new node between existing ones, or fix ordering). For cross-branch move use move_node / batch_move_nodes instead.',
      parameters: {
        type: 'object',
        properties: {
          uid: { type: 'string', description: 'uid of the node to reposition' },
          index: { type: 'number', description: '0-based target index among siblings (after removing the node); e.g. 1 places it second' },
          before_uid: { type: 'string', description: 'Place the node immediately before this sibling uid' },
          after_uid: { type: 'string', description: 'Place the node immediately after this sibling uid' }
        },
        required: ['uid']
      }
    }
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Create/configure a new MCP server. transport=http needs url; transport=stdio needs command (+ optional args array).',
      parameters: {
        type: 'object',
        properties: {
          name: { type: 'string', description: 'MCP server name (unique)' },
          transport: { type: 'string', enum: ['http', 'stdio'], description: 'http (SSE/streamable) or stdio (local process)' },
          url: { type: 'string', description: 'HTTP/SSE endpoint url (when transport=http)' },
          command: { type: 'string', description: 'Command to launch (when transport=stdio), e.g. npx' },
          args: { type: 'array', items: { type: 'string' }, description: 'Command args (when transport=stdio)' }
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Create/update a custom tool by writing its tool.json manifest and tool.js source into the custom-tools directory. The tool runs in an isolated subprocess with a path-whitelisted file API and optional PowerShell. Use when no built-in tool solves the problem and you need a NEW reusable capability.',
      parameters: {
        type: 'object',
        properties: {
          id: { type: 'string', description: 'Tool id (lowercase kebab-case, e.g. my-fetcher). Uniquely identifies the tool.' },
          ...
          code: { type: 'string', description: 'Complete tool.js source (ESM). Must export `export async function execute(args, context) { ... }` returning an object with a `message` string. Available: context.file.{readText,writeText,exists}, context.http.fetch, context.app.' },
          parameters: { type: 'object', description: 'JSON Schema for the tool\'s arguments (e.g. {"type":"object","properties":{"url":{"type":"string"}},"required":["url"]})' },
          powershell: { type: 'boolean', description: 'Set true ONLY if this tool genuinely needs to run PowerShell (context.shell.powerShell). Defaults false for safety.' }
        },
  ...
  },
  {
    ...
    function: {
      name: 'ask',
      description: 'Ask the user a question with clickable choices and wait for their answer. Use this when you need clarification, confirmation, or a decision from the user before you can proceed. The user will see buttons and pick one. IMPORTANT: After calling this tool, the current turn ends — the user\'s answer will come in the next message, so you should NOT continue reasoning after calling ask.',
      parameters: {
        type: 'object',
        properties: {
          question: { type: 'string', description: 'The question to ask the user (short, 1-2 sentences)' },
          type: {
            type: 'string',
            enum: ['single', 'multi'],
            description: 'single = pick one option, multi = can select multiple options. Default: single.'
          },
          options: {
            type: 'array',
            items: { type: 'string' },
            description: 'List of choice labels (2-8 options). Keep each option short and clear.'
          }
        },
  ...
  },
  {
    ...
    function: {
      ...
      description: 'Read one or more local documents (PDF/DOCX/PPTX/XLSX/XLS/CSV/MD/TXT) and generate a new .smm mindmap file IN THE BACKGROUND, without opening or switching the current canvas. For a SINGLE document pass file_path; for MULTIPLE documents that must be MERGED into ONE mindmap, pass file_paths=[...] in a single call. Do NOT read files one by one with read_local_file and then call generate_mindmap — use this tool with file_paths instead.',
      parameters: {
        type: 'object',
        properties: {
          file_path: { type: 'string', description: 'Absolute source document path (single document)' },
          file_paths: { type: 'array', items: { type: 'string' }, description: 'Absolute paths of multiple source documents to merge into one mindmap (use instead of file_path when integrating several files)' },
...
// ===== 导图写操作串行锁 =====
// AI 可能一次并行发出多个导图修改类工具调用（如多个 expand_node），
// 并发执行会让 renderer.root 短暂失效，报「思维导图未初始化」。
// 用模块级 Promise 链把导图写操作串行化，从根上消除并发冲突。
let mindMapWriteChain = Promise.resolve()
const MINDMAP_WRITE_TOOLS = new Set([
  'expand_node', 'add_child_nodes', 'insert_parent_node', 'delete_node',
  'move_node', 'batch_move_nodes', 'merge_nodes', 'duplicate_nodes',
  'update_node_text', 'set_node_style', 'batch_node_actions', 'batch_text_style',
  'sort_children', 'reorder_nodes', 'set_node_note', 'summarize_node',
  'insert_sibling_node', 'insert_child_node', 'clear_mindmap', 'generate_mindmap',
  'save_mindmap', 'set_mindmap_data', 'new_mindmap', 'undo', 'redo',
  'remove_node_only', 'copy_node', 'cut_node', 'paste_node'
])

// 导图写操作后兜底重渲染：AI 连续添加节点时，execCommand 内部的 render 会走
// 异步批处理队列并被合并（hasWaitRendering）。合并渲染会复用节点，但连线对象
// 可能仍指向已被 clearDraw 清除的旧 DOM，导致「框架线缺失」。
// 等队列落定后 reRender 一次重建连线（与切换标签后的兜底逻辑一致）。
function scheduleLineRecoveryRerender(mindMap) {
  try {
    let mm = mindMap
    if (!mm) {
      const store = useMindMapStore()
      mm = store && store.mindMapInstance ? store.mindMapInstance : null
    }
    if (!mm || !mm.renderer || !mm.renderer.hasWaitRendering) return
    setTimeout(() => {
      try { if (mm && mm.reRender) mm.reRender() } catch (e) { /* 忽略 */ }
    }, 300)
  } catch (e) { /* 忽略 */ }
}

// 等待渲染器落到稳定态（renderer.root 非空且未在渲染中）。
// execCommand 触发的 render 是异步的：render() 先把 renderer.root 置 null，再异步 doLayout 重建。
// 若下一个写工具（串行队列里的）在上一次 render 尚未完成时读取 renderer.root，会误报「思维导图未初始化」。
// 这里轮询等待 root 恢复，最多 1.5 秒，避免串行队列里的连续写操作互相踩踏。
async function waitForRendererSettle(mindMap) {
  try {
    let mm = mindMap
    if (!mm) {
      const store = useMindMapStore()
    ...
    while (Date.now() < deadline) {
      const r = mm.renderer
      if (r.root && !r.isRendering) return
...
export async function handleToolCall(toolCall, mindMap, activeNode, extraHandlers = {}) {
  ...
  if (MINDMAP_WRITE_TOOLS.has(name)) {
    ...
    let release
    mindMapWriteChain = new Promise((res) => { release = res })
    await prev
    try {
      const result = await handleToolCallInner(toolCall, mindMap, activeNode, extraHandlers)
      scheduleLineRecoveryRerender(mindMap)
      return result
    } finally {
      // 等本次渲染落定后再放行下一个写工具，避免连续的 expand_node 读到 root=null
      await waitForRendererSettle(mindMap)
      release()
    }
  }
...
` + contentText.slice(0, 16000)
    ...
    case 'research_to_mindmap': {
      ...
      try {
        // 兼容模型误用 content / markdown_text 等别名传参（历史故障：模型把全文塞进 content 字段，导致“Markdown 内容为空”）
        const md = args.markdown ?? args.content ?? args.markdown_text ?? args.text ?? ''
        if (!md || !String(md).trim()) return { success: false, message: 'Markdown 内容为空' }
        const treeData = parseMarkdownToTree(String(md))
        ensureRichText(treeData)
          ...
          setTimeout(() => {
            try {
              ...
              if (fresh.length > 0) {
                ...
                // 诊断：检测新节点是否正确设置了 richText（数据创建时已带 richText:true，
                // 若此处检测到异常则打印警告，便于后续定位"显示HTML标签"问题的根因；
                // 不主动 setData 避免触发多次重渲染导致节点重叠）
                if (fresh.length > 0) {
                  const sample = fresh[0]
                  try {
                    const txt = sample.getData?.('text')
                    const rt = sample.getData?.('richText')
                    if (txt && !rt && /^<p/i.test(String(txt))) {
                      console.warn('[add_child_nodes] 新节点 richText 未正确设置，文本:', String(txt).slice(0, 100))
                    }
                  } catch (_) {}
                }
              ...
              // 注意：INSERT_MULTI_CHILD_NODE 内部已触发渲染，不再额外 render()，
              // 避免双重渲染导致节点重叠/重影
    ...
    case 'export_mindmap': {
      try {
        ...
        if (format === 'xmind') {
          ...
          zip.file('metadata.json', JSON.stringify({ dataStructureVersion: '2.0', creator: { name: 'my-mindmap agent', version: '4.18.0' } }))
...
${block}`
    ...
    case 'search_knowledge_base': {
      try {
        ...
        if (results.length === 0) {
          return { success: true, message: `在知识库中搜索"${query}"未找到匹配内容。（本地知识库无相关内容，请如实告知用户，不要编造或猜测答案）` }
        }
        const formatted = results.map((r, i) => {
          const refLink = r.nodeUid ? `mindmap-node:${r.filePath}:${r.nodeUid}` : `mindmap-file:${r.filePath}`
          return `${i + 1}. 文件：${r.fileName}\n   内容：${(r.snippet || '').replace(/<[^>]+>/g, '')}\n   引用链接：${refLink}`
        }).join('\n\n')
        return {
          success: true,
          message: `知识库搜索"${query}"找到 ${results.length} 条结果：\n\n${formatted}\n\n【引用规范】回答中引用来源时，请使用 Markdown 链接格式 [文件名](引用链接)，例如 [某文件](mindmap-node:路径:uid)。这样用户点击即可跳转到对应文件/节点。`,
          searchResults: results
        }
      } catch (e) {
    ...
    case 'semantic_search': {
      try {
        ...
        if (ranked.length > 5) {
          try {
            ...
            const items = pool.map((r, i) => ({
              idx: i,
              text: String(r.snippet || '').replace(/<[^>]+>/g, '').trim().slice(0, 120),
              file: r.fileName || ''
            }))
            const choice = await aiService.chat(
              `用户问题：${query.trim()}\n\n候选片段：\n${items.map(it => `[${it.idx}] ${it.file}：${it.text}`).join('\n')}\n\n请按相关度输出候选编号的重排顺序（JSON 数组，最相关在前）。`,
              '你是检索结果重排助手，根据用户问题与候选片段的相关度排序。只输出 JSON 数组，不要解释。',
              null,
              { responseFormat: 'json', thinking: false, max_tokens: 200 }
            )
            const raw = (choice?.message?.content || '').trim()
            const jsonText = raw.replace(/```(?:json)?/gi, '').replace(/```/g, '').trim()
            let order = null
            try { const p = JSON.parse(jsonText); order = Array.isArray(p) ? p : null } catch { const m = jsonText.match(/\[[\d,\s]+\]/); if (m) { try { order = JSON.parse(m[0]) } catch {} } }
            if (Array.isArray(order) && order.length > 0) {
              const seen = new Set(order.map(i => Number(i)))
              const reordered = order.map(i => pool[Number(i)]).filter(Boolean)
              rankedResults = [...reordered, ...pool.filter((_, i) => !seen.has(i)), ...ranked.slice(15)].map((r, i) => ({ ...r, ref: i + 1 }))
            }
          } catch (e) {
            console.warn('[语义检索] LLM 重排失败，降级原排序:', e)
          }
        }
        ...
        if (rankedResults.length === 0) {
          return {
            success: true,
            message: `语义检索"${query}"（关键词：${terms.join('、')}）未找到匹配内容。（本地知识库无相关内容，请如实告知用户，不要编造或猜测答案。可建议用户换关键词重试，或确认相关文件已打开/索引）`
          }
        }
        ...
        const fileSections = [...byFile.entries()].map(([fileName, items], fi) => {
          const nodes = items.slice(0, 10).map((r) => {
            ...
            const refLink = r.nodeUid ? `mindmap-node:${r.filePath}:${r.nodeUid}` : `mindmap-file:${r.filePath}`
            return `   [KB${r.ref}] ${r.snippet}（${hitInfo}；相关度：${r.score}）\n   引用链接：${refLink}`
          }).join('\n')
        ...
        return {
          success: true,
          message: `语义检索"${query}"（关键词：${terms.join('、')}）共命中 ${rankedResults.length} 条（按相关度排序；回答时必须引用 [KB编号]）：\n\n${fileSections}\n\n【引用规范】回答中引用来源时，请使用 Markdown 链接格式 [文件名](引用链接)，例如 [某文件](mindmap-node:路径:uid)。这样用户点击即可跳转到对应文件/节点。`,
    ...
    case 'search_across_files': {
      try {
        const query = args.query || ''
        const maxResultsPerFile = Number(args.maxResultsPerFile) || 3
        const maxFiles = Number(args.maxFiles) || 5
        ...
        if (results.length === 0) {
          return {
            success: true,
            message: `跨文件搜索"${query}"未找到匹配内容。（本地知识库无相关内容，请如实告知用户，不要编造或猜测答案。可尝试换关键词，或使用 get_related_files 查看已知关联文件）`
          }
        }
        ...
        for (const r of results) {
          const fp = r.filePath || r.fileName || ''
          if (!byFile.has(fp)) {
            byFile.set(fp, {
              filePath: fp,
              fileName: r.fileName,
        ...
        // 构建按文件分组的结果文本
        const fileSections = topFiles.map((f, fi) => {
          const topHits = f.hits.slice(0, maxResultsPerFile)
          const hitsText = topHits.map((r, hi) => {
            const refLink = r.nodeUid
              ? `mindmap-node:${f.filePath}:${r.nodeUid}`
              : `mindmap-file:${f.filePath}`
            const snippet = (r.snippet || '').replace(/<[^>]+>/g, '').trim()
            return `   [${fi + 1}-${hi + 1}] ${snippet}\n   引用：${refLink}`
          }).join('\n\n')
          const more = f.hits.length > maxResultsPerFile
            ? `\n   ...等共 ${f.hits.length} 处命中`
            : ''
          return `${fi + 1}. 📄 ${f.fileName}（${f.hits.length} 处命中）\n${hitsText}${more}`
        }).join('\n\n')

        // 关联文件推荐
        let relatedSection = ''
        if (includeRelated && topFiles.length > 0) {
          const sourcePaths = topFiles.map(f => f.filePath).filter(Boolean)
          const relatedFiles = expandRelatedFiles(sourcePaths, 5)
          if (relatedFiles.length > 0) {
            relatedSection = `\n\n🔗 推荐相关文件（可进一步搜索扩展）：\n${relatedFiles.map((rf, i) => {
              return `  ${i + 1}. ${rf.name}（${rf.reason}，相关度：${Math.round(rf.weight * 100)}%）\n     路径：${rf.path}`
            }).join('\n')}`
          }
        }
        ...
        return {
          success: true,
          message: `跨文件搜索"${query}"在 ${byFile.size} 个文件中找到 ${results.length} 处匹配：\n\n${fileSections}${relatedSection}\n\n【使用建议】\n1. 如需深入了解某文件，用 read_mindmap_file 读取完整内容\n2. 如需扩展搜索范围，用 get_related_files 查询更多关联文件\n3. 引用来源时使用 [文件名](引用链接) 格式`,
          searchResults: topFiles.map(f => ({
            filePath: f.filePath,
            fileName: f.fileName,
            hitCount: f.hits.length,
            hits: f.hits.slice(0, maxResultsPerFile)
          })),
          relatedFiles: includeRelated ? expandRelatedFiles(topFiles.map(f => f.filePath).filter(Boolean), 5) : []
        }
      } catch (e) {
        return { success: false, message: `跨文件搜索失败: ${e.message}` }
      }
    }
    ...
    case 'get_related_files': {
      try {
        ...
        const types = Array.isArray(args.relationTypes) && args.relationTypes.length > 0
          ? args.relationTypes
          : null
        ...
        const rels = getRelatedFiles(filePath, { types, limit })

        if (rels.length === 0) {
          return {
            success: true,
            message: `未找到与 "${filePath}" 相关联的文件。（该文件可能还没有建立关联关系，或相关文件尚未被索引）`
          }
        }

        const typeLabel = {
          [RELATION_TYPES.REFERENCE]: '引用了该文件',
          [RELATION_TYPES.REFERENCED_BY]: '被该文件引用',
          [RELATION_TYPES.SIMILAR]: '主题相似',
          [RELATION_TYPES.MANUAL]: '手动关联'
        }

        const relsText = rels.map((r, i) => {
          const name = r.target.split('/').pop() || r.target
          const label = typeLabel[r.type] || r.type
          return `${i + 1}. 📄 ${name}\n   关系：${label}（相关度 ${Math.round(r.weight * 100)}%）\n   路径：${r.target}`
        }).join('\n\n')

        return {
          success: true,
          message: `找到 ${rels.length} 个关联文件：\n\n${relsText}\n\n【使用建议】如需在这些文件中搜索内容，可使用 search_across_files 或 search_knowledge_base 工具，配合相关关键词进一步检索。`,
          relatedFiles: rels.map(r => ({
            path: r.target,
        ...
        return { success: false, message: `获取关联文件失败: ${e.message}` }
      }
    }
    ...
    case 'create_mcp_server': {
      try {
        if (!args.name) return { success: false, message: '请提供 MCP 名称（name）' }
        const transport = String(args.transport || 'http').toLowerCase()
        if (!['http', 'stdio'].includes(transport)) return { success: false, message: 'transport 只支持 http 或 stdio' }
        if (transport === 'http' && !args.url) return { success: false, message: 'transport=http 需要提供 url' }
        if (transport === 'stdio' && !args.command) return { success: false, message: 'transport=stdio 需要提供 command' }
        if (!window.electronAPI?.mcp?.create) return { success: false, message: 'MCP 创建功能不可用' }
        let argv = []
        try { argv = Array.isArray(args.args) ? args.args : JSON.parse(args.args || '[]') } catch { argv = [] }
        const server = await window.electronAPI.mcp.create({
          name: args.name,
          transport,
          url: args.url,
        ...
        const howTo = transport === 'http'
          ? `可通过 list_mcp_tools(serverId="${server?.id || ''}") 查看工具，再用 mcp_call_tool 调用`
          : `可通过 list_mcp_tools 查看工具，再用 mcp_call_tool 调用`
        return { success: true, message: `已创建 MCP 服务「${args.name}」（transport=${transport}）。${howTo}。`, server }
      } catch (e) { return { success: false, message: `创建 MCP 服务失败: ${e.message}` } }
    }
    ...
    case 'write_custom_tool': {
      ...
      } catch (e) { return { success: false, message: `创建自定义工具失败: ${e.message}` } }
    }
    ...
    case 'reorder_nodes': {
      try {
        if (!mindMap) return { success: false, message: '当前没有打开的思维导图。请先调用 find_local_file(exts=["smm"]) 搜索本地导图文件（自动覆盖桌面/文档/下载/默认保存目录），再用 read_mindmap_file(filePath=...) 直接读取文件内容后继续。' }
        if (!args.uid) return { success: false, message: '请提供要移动的节点 uid' }
        // 直接操作数据树（setData 全量刷新，与 sort_children 一致，简单可靠）
        const treeData = mindMap.getData()
        const findNodeWithParent = (root, targetUid, parent = null) => {
          if (!root) return null
          if (root?.data?.uid === targetUid) return { node: root, parent }
          for (const c of root?.children || []) {
            const r = findNodeWithParent(c, targetUid, root)
            if (r) return r
          }
          return null
        }
        const found = findNodeWithParent(treeData, args.uid)
        if (!found) return { success: false, message: `数据树中未找到 uid=${args.uid} 的节点` }
        if (!found.parent) return { success: false, message: '根节点无法在同层级重排' }
        const siblings = found.parent.children || []
        const curIndex = siblings.indexOf(found.node)
        if (curIndex === -1) return { success: false, message: '节点不在父节点的子列表中' }
        siblings.splice(curIndex, 1)

        // 目标位置：index（0-based，移除后索引）/ before_uid / after_uid，三选一
        let targetIndex = -1
        if (Number.isInteger(args.index)) {
          targetIndex = args.index
        } else if (args.before_uid) {
          targetIndex = siblings.findIndex(c => c?.data?.uid === args.before_uid)
          if (targetIndex === -1) return { success: false, message: `未找到 before_uid=${args.before_uid} 的兄弟节点` }
        } else if (args.after_uid) {
          const ai = siblings.findIndex(c => c?.data?.uid === args.after_uid)
          if (ai === -1) return { success: false, message: `未找到 after_uid=${args.after_uid} 的兄弟节点` }
          targetIndex = ai + 1
        }
        if (targetIndex < 0) return { success: false, message: '请提供 index / before_uid / after_uid 之一作为目标位置' }
        if (targetIndex > siblings.length) targetIndex = siblings.length
        siblings.splice(targetIndex, 0, found.node)
        mindMap.setData(treeData)

        // 重新定位到移动后的节点，方便用户看到结果
        const moved = typeof mindMap.renderer.findNodeByUid === 'function' ? mindMap.renderer.findNodeByUid(args.uid) : null
        if (moved && typeof mindMap.renderer.moveNodeToCenter === 'function') {
          mindMap.renderer.moveNodeToCenter(moved)
        }
        return { success: true, message: `已重排节点位置（移动到兄弟节点中的第 ${targetIndex + 1} 位）` }
      } catch (e) {
        return { success: false, message: `重排节点失败: ${e.message}` }
      }
    }
    ...
    case 'convert_doc_to_mindmap': {
      try {
        ...
        if (!filePath && filePaths.length === 0) {
          return { success: false, message: '请提供 file_path（单个文档）或 file_paths（多个文档数组）' }
        }
        if (typeof extraHandlers?.convertDocToMindmap !== 'function') {
          return { success: false, message: '当前环境不支持文档转思维导图工具' }
        }
        const reply = await extraHandlers.convertDocToMindmap({ ...args, file_path: filePath, file_paths: filePaths })
        const text = String(reply || '').trim()
        if (!text) return { success: false, message: '文档转思维导图未返回有效结果' }
        // 转换在后台实例执行，失败会以「转换失败：…」返回；只有真成功才标记 success，避免静默假成功误导模型。
        if (text.startsWith('转换失败') || text.startsWith('请提供 file_path')) {
    ...
    case 'memory': {
      try {
        ...
        if (action === 'save') {
          ...
          // 防误记：一次性任务要求（含任务动词且无长期意图关键词）不保存为长期记忆
          const isTaskVerb = /生成|创建|导出|转换|整理|挖空|拆分|合并|删除|重命名|移动|复制|制作|总结|概括/.test(content)
          const hasLongTermIntent = /总是|以后都|今后|记住|我喜欢|我习惯|我偏好|我的习惯|我的偏好|永久|长期/.test(content)
          if (isTaskVerb && !hasLongTermIntent) {
            return { success: false, message: '该内容看起来是一次性任务要求，不是稳定的长期偏好，未保存。只有当用户明确表达长期意图（如「记住…/以后都…/我总是…」）时才保存。' }
          }
    ...
    case 'ask': {
      try {
        ...
        const type = args.type === 'multi' ? 'multi' : 'single'

        if (!question.trim()) {
          return { success: false, message: 'ask 工具缺少 question 参数' }
        }
        if (options.length < 2) {
          return { success: false, message: 'ask 工具至少需要 2 个选项' }
        }

        // 构造 <ask> 标签格式的内容，前端会自动解析为可点击按钮
        const askJson = JSON.stringify({
          type,
          question,
          options: options.slice(0, 8)
        })
        const askBlock = `<ask>${askJson}</ask>`
        ...
        return { success: false, message: `ask 工具失败: ${e.message}` }
      }
    }
