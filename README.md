...
- 左侧是**文件树 / 大纲 / 复习计划 / 标签**四合一侧边栏；
- 中间是**思维导图画布**（基于 `simple-mind-map`，支持主题、布局、缩略图、全屏、Obsidian 式分屏）；
...
更进一步的 **关联图（Graph View）** 功能，把导图节点之间的关系以力导向图可视化呈现，一眼看清知识脉络；**分屏（Split View）** 则让你在同一界面并排对比、联动编辑多份导图。
...
| 主题/布局/视图 | 切换主题、布局、大纲/脑图/关联图/复习多模式、缩放、撤销重做 |

### 2. 三种视图模式 + 关联图

- **思维导图模式**：完整画布，支持全屏、缩略图、多种主题与结构布局。
- **大纲模式**：所见即所得的大纲编辑，`Shift+Enter` 节点内换行，可导入 Markdown / XMind，与画布双向同步。
- **复习模式**：把节点加入**艾宾浩斯遗忘曲线复习计划**（1/3/7/15/31 天记忆周期），到点系统通知提醒。
- **🔗 关联图模式（v3.x 新增）**：将当前导图节点及其之间的**关联线（associative line）**渲染为力导向关系图。点击关联图节点可一键切回导图视图并高亮定位该节点，帮你快速把握知识骨架与概念交叉关系。支持**导出交互式关联图 HTML**，单独分享给他人查看。

### 3. 分屏对比（Obsidian 式，v3.x 新增）

- **多窗格同屏**：一个界面可并排打开多份导图/文档，左右或上下自由分割。
- **拖拽分屏**：把顶部标签拖到另一个窗格边缘即可按方向（左/右/上/下）分屏；拖到中间可交换两个窗格位置。
- **分割条调比例**：拖动窗格之间的分割条即可实时调整各窗格大小。
- **联动不串台**：每个 AI 任务仍绑定目标文件（fileId），分屏时切换文件也不会改错图；同一份文件也可出现在多个窗格（Obsidian 行为）。
...
- 支持导入/导出 **XMind、Markdown、PDF、图片、OPML、FreeMind** 等格式。
...
### 10. 自定义工具 / MCP / 技能 / 远程服务

- **用户自定义工具系统**：在不改主程序代码的情况下，按规范新增本地工具（`custom-tools/<tool-id>/` 下的 `tool.json` + `tool.js`），AI 可自动识别调用。
- **MCP 服务器管理**：接入 stdio 型 MCP（Model Context Protocol）工具服务器，扩展 AI 可调用的外部能力，设置页启停实时生效。
- **用户自定义技能管理**：本地创建/编辑/启用 AI 技能（Skills），由 AI 按需自动调用。
- **本地 HTTP / WebSocket 远程服务**：默认端口 17800，带访问 Token 鉴权（60 天有效），支持浏览器/外部设备远程查看与操控导图画面；并对外提供 `skills/my-mindmap-agent-api/SKILL.md` 供其他 AI Agent 通过 API 调用本应用。

### 11. 桌面宠物猫

- `cherry-cat-pet/` 模块，含 Python 生成的精灵图动画、宠物配置与问答素材，为学习增添乐趣。

### 12. 工程与体验细节

- **意图规范化改写**：发送消息前先让 AI 做意图识别与改写，提升弱模型下的指令遵循率。
- **自动保存 / 软重启**：关闭前自动落盘；界面卡死用"刷新"按钮软重启恢复。
- **安全**：主进程对渲染层 IPC 调用做发送方校验，防 XSS 越权；API Key 仅存于本地、由主进程注入请求头，渲染进程不持有明文。
...
my-mindmap-agent/
├── src/                      # Vue 3 前端（渲染进程）
│   ├── App.vue               # 主框架：导航栏 / Tab 栏 / 分屏布局 / 多视图（导图·大纲·关联图·复习）
│   ├── components/           # 各 UI 组件（导图编辑器、大纲、关联图、复习、聊天、设置…）
│   ├── services/             # AI 服务、工具调度、飞书/微信/知识库/OCR/搜索/定时任务
│   ├── stores/               # Pinia 状态（AI、思维导图实例）
│   ├── utils/                # 挖空、复习计划、关联图导出、Markdown/XMind 解析、PDF、上下文窗口…
...
**技术栈**：Vue 3 · Pinia · Element Plus · Vite 5 · Electron 28 · electron-builder · simple-mind-map · sql.js · tesseract.js（OCR）· jszip · pdfjs-dist · force-graph（关联图）· @larksuiteoapi/node-sdk（飞书）· ws（WebSocket）。
...
前往 **[Releases](../../releases)** 页面，下载 `my-mindmap-agent-Setup-3.1.0.exe`：
...
> 注意：联网搜索、OCR、飞书/微信、定时任务、分屏、关联图等能力依赖 Electron 主进程，**仅在 `npm run electron:dev` / 打包后的桌面端可用**，纯 `npm run dev` 浏览器模式不可用。
...
4. 试试新能力：
   - 点右下角 **关联图** 按钮，切换力导向关系视图，看清节点间逻辑脉络；
   - 把顶部标签拖到另一份导图窗格边缘，**分屏**并排对比、联动编辑。

---

## 七、常见问题

- **AI 改错了图？** 每个 AI 任务都绑定了发起时的文件，只动它该动的图；分屏下也互不串台。
- **关联图看什么？** 关联图把节点之间的关联线（如"导致/对比/相关"）可视化为力导向图，方便发现概念交叉与知识骨架；点击节点可跳回导图高亮定位。
